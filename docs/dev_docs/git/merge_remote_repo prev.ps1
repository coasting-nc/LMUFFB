...
.EXAMPLE
    .\merge_remote_repo.ps1 -RemoteRepoUrl "https://github.com/user/repo" -RemoteAlias "user-repo" -AuthorEmail "custom@email.com"
...
Write-Step "Adding remote repository '$RemoteRepoUrl' as '$RemoteAlias'..."
git remote add $RemoteAlias $RemoteRepoUrl
if ($LASTEXITCODE -ne 0) {
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "4. Re-run this script OR continue manually with the guide" -ForegroundColor Yellow
...
Squashed merge of all changes from $RemoteRepoUrl
"@
