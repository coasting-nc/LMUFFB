...
## 7. CI Build Optimization: Compiler-Level Caching (v0.7.244)

### 7.1 Objective
To significantly reduce the duration of GitHub Actions CI runs by avoiding the redundant compilation of vendor and core modules.

### 7.2 Strategy: `Sccache` vs `Ccache`
We have implemented a dual-layer compiler caching strategy to handle both Windows and Linux environments:

*   **Windows (Sccache)**: Transitioned from `ccache` to `sccache` (via `mozilla/sccache-action`). `sccache` is better suited for MSVC and correctly caches the intermediate objects and debug symbols generated by the Windows compiler.
*   **Linux (Ccache)**: Optimized the existing `ccache` implementation by ensuring it is correctly passed to CMake via compiler launchers.

### 7.3 Integration Checklist
- [x] **Compiler Launchers**: Configured CMake with `-DCMAKE_C_COMPILER_LAUNCHER` and `-DCMAKE_CXX_COMPILER_LAUNCHER`.
- [x] **Stable Cache Keys**: Shifted from SHA-based keys (which forced misses) to content-based hashes using `hashFiles('CMakeLists.txt', 'vendor/**')`.
- [x] **No-Clean Policy**: Removed `--clean-first` from the Windows CI build step to allow incremental reuse of cached object files within the same job's filesystem.
