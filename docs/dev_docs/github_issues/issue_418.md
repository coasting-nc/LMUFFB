# Strong Wheel Oscillations when in the Pits #418
...
Here is the exact implementation to make them completely independent.
We will separate the math into two distinct forces: `ctx.gyro_force` (which turns off in the menus) and `ctx.stationary_damping_force` (which stays active in the menus/pits but fades to exactly zero when driving).
