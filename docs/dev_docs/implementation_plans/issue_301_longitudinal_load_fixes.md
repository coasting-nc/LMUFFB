...
- **Formula Refactor**: The effect is internally calculated as `ctx.long_load_force = base_steer_force * (dw_factor_applied - 1.0) * ctx.speed_gate` for diagnostic visualization only. The main output is `output_force = base_steer_force * dw_factor_applied * ctx.speed_gate`.
