...
**GitHub Issue:** #154 - Implement FFB Strength Normalization Stage 3 - Tactile Haptics Normalization (Static Load + Soft-Knee)
...
### Unforeseen Issues
- **Test Regressions:** Several existing unit tests (`test_ffb_smoothstep`, `test_ffb_features`, `test_ffb_slip_grip`, `test_ffb_lockup_braking`, `test_ffb_road_texture`) relied on the legacy linear scaling of tactile effects and lacked appropriate initialization for the new stateful smoothing. These tests failed initially because the `m_smoothed_tactile_mult` would start at 1.0 and evolve differently than the hardcoded expectations, or because of interaction with the new bottoming logic.
- **Negative Load Factors:** During code review, it was identified that while tire load is physically positive, telemetry noise could theoretically produce negative values, which would lead to negative multipliers. A safety clamp was added to ensure the load factor remains non-negative.

### Plan Deviations
- **InitializeLoadReference Reset:** The plan initially missed adding the reset of the new state variables (`m_static_load_latched` and `m_smoothed_tactile_mult`) to `InitializeLoadReference`. This was identified as critical for ensuring a clean slate when switching cars and was added to the implementation.
- **Test Tweaks:** In some existing tests, it was necessary to explicitly disable the bottoming effect or force the tactile multiplier to 1.0 to isolate the specific logic being verified (e.g., speed gating or road texture delta clamping).

### Challenges
- **Balancing Smoothing vs. Response:** Choosing the 100ms time constant for the tactile multiplier requires a balance between rejecting high-frequency load noise and responding quickly to significant weight transfers. The tests verify that the settling time is reasonable for 400Hz operation.
- **Mocking Telemetry for State Transitions:** Writing unit tests that trigger the "transition" region of the soft-knee required careful construction of telemetry data to hit specific ratios (e.g., exactly 1.5x static load).

### Recommendations
- **Telemetry Replay in Tests:** Future stages could benefit from a "telemetry replay" test helper that can feed recorded CSV data into the engine to verify normalization behavior against real-world scenarios.
- **GUI Monitoring:** Adding a small "Tactile Mult" indicator to the GUI graphs would help users visualize how much compression is being applied during high-downforce cornering.
