...
- **Pushing Restrictions**: While I can perform local git operations, pushing to a remote branch is handled **exclusively** through the `submit` tool. Direct `git push` commands are explicitly **blocked** in the `run_in_bash_session` environment to ensure the submission process follows the required orchestration.
