...
Your task is to implement the **`RearAxleConfig`** category. 
...
1. **Consistency Test:** Hardcode a specific telemetry state and specific `RearAxleConfig` values. Run this test on the *current* codebase to get the exact `final_force` output, hardcode that expected value into the test, and `ASSERT_NEAR` against it.
2. **Round-Trip Test:** Create a `Preset` with wild `RearAxleConfig` values, call `Apply(engine)`, extract it back with `UpdateFromEngine(engine)`, and `ASSERT_TRUE` that the original and extracted presets are `Equals()`.
3. **Validation Test:** Create a `Preset` with malicious out-of-bounds `RearAxleConfig` values. Call `Apply()`, and assert that the engine clamped them to the safe ranges defined in `RearAxleConfig::Validate()`.
...
1. **Scope:** Work ONLY on `RearAxleConfig`. Do not touch `BrakingConfig`, `VibrationConfig`, etc.
...
1. **Micro-Commits:** You must commit your work at every stable step. Do not wait until the entire task is finished. 
   ...
   * *Checkpoint 2:* Commit immediately after defining the `RearAxleConfig` struct in the header files.
