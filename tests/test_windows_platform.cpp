...
static void test_window_title_extraction() {
  ...
  // We expect something, even if it's "Unknown" (though on Windows it should
  // work)
...
static void test_window_always_on_top_behavior() {
  ...
  // Added WS_VISIBLE as SetWindowPos might behave differently for hidden
  // windows in some environments
  HWND hwnd =
      CreateWindowA("STATIC", "TestWindow", WS_OVERLAPPEDWINDOW | WS_VISIBLE, 0,
  ...
  BOOL success1 = ::SetWindowPos(hwnd, HWND_TOPMOST, 0, 0, 0, 0,
  ...
  BOOL success2 = ::SetWindowPos(hwnd, HWND_NOTOPMOST, 0, 0, 0, 0,
                                 SWP_NOMOVE | SWP_NOSIZE | SWP_FRAMECHANGED);
...
static void test_preset_management_system() {
  ...
  // 4. Add user preset (this will save to config.yaml by default)
...
static void test_slider_precision_display() {
  ...
  // Format should be "Q: %.2f" to show 0.01 changes
  {
    ...
    std::cout << "  Filter Width: " << result1 << " -> " << result2
              << std::endl;
  }

  // Test Case 2: Percentage sliders - Range 0-2.0, step 0.01
  // Format should be "%.1f%%" to show 0.5% changes (0.01 * 100 = 1.0, but
  // displayed as 0.5% of 200%)
  {
    float value = 1.00f;
    char buf[64];
...
static void test_slider_precision_regression() {
  ...
  // Bug: Was "%.0f Hz" which didn't show small adjustments
  {
    ...
    // The displayed strings should be different
    ASSERT_TRUE(result1 != result2);
    std::cout << "  Target Frequency: " << result1 << " -> " << result2
              << std::endl;
  }
  ...
  // Bug: Was using pre-calculated percentage with buffer scope issues
  {
    ...
    ASSERT_TRUE(result1 != result2);
    std::cout << "  Understeer Effect: " << result1 << " -> " << result2
              << std::endl;
  }
...
static void test_latency_display_regression() {
  ...
  // This test verifies the latency display feature that was accidentally
  // removed during the GUI overhaul and restored in v0.4.50 Updated in v0.4.51
  // to include rounding and new layout
  ...
  // Formula: lat_ms = (1.0 - sop_smoothing_factor) * 100.0 + 0.5 (Rounding)
  {
    ...
    std::cout << "    Low latency: " << lat_ms_low << " ms (green)"
              << std::endl;
  ...
  {
    std::cout << "  Testing Slip Angle Smoothing latency calculation..."
              << std::endl;
    ...
    std::cout << "    Low latency: " << slip_ms_low << " ms (green)"
              << std::endl;
    ...
    std::cout << "    High latency: " << slip_ms_high << " ms (red)"
              << std::endl;
    ...
    std::cout << "    Boundary latency: " << slip_ms_boundary << " ms (red)"
              << std::endl;
  }
  ...
  // Test Case 4: Display Format Verification
  {
    ...
    int lat_ms = 14;
    char buf[64];
    snprintf(buf, 64, "Latency: %d ms - %s", lat_ms,
             (lat_ms < 15) ? "OK" : "High");
...
static void test_single_source_of_truth_t300_defaults() {
  ...
  std::cout << "  Test 1: Captured reference defaults from Preset struct"
            << std::endl;

  // Test 2: Verify FFBEngine initialization via ApplyDefaultsToEngine() matches
  {
    std::cout << "  Test 2: FFBEngine initialization consistency..."
              << std::endl;
    FFBEngine engine;
    Preset::ApplyDefaultsToEngine(engine);
    ...
    ASSERT_TRUE(engine.m_slide_texture_enabled ==
                reference_defaults.slide_enabled);
    ...
    ASSERT_TRUE(engine.m_rear_align_effect ==
                reference_defaults.rear_align_effect);
    ...
    ASSERT_TRUE(engine.m_optimal_slip_angle ==
                reference_defaults.optimal_slip_angle);
    ASSERT_TRUE(engine.m_slip_angle_smoothing ==
                reference_defaults.slip_smoothing);
    ASSERT_TRUE(engine.m_sop_smoothing_factor ==
                reference_defaults.sop_smoothing);
    ASSERT_TRUE(engine.m_yaw_accel_smoothing ==
                reference_defaults.yaw_smoothing);
    ASSERT_TRUE(engine.m_chassis_inertia_smoothing ==
                reference_defaults.chassis_smoothing);
    ASSERT_TRUE(engine.m_gyro_smoothing == reference_defaults.gyro_smoothing);
    ASSERT_TRUE(engine.m_steering_shaft_smoothing ==
                reference_defaults.steering_shaft_smoothing);
  ...
  // Test 3: Verify "Default" preset from LoadPresets() matches
  {
    ...
    ASSERT_TRUE(default_preset.oversteer_boost ==
                reference_defaults.oversteer_boost);
    ASSERT_TRUE(default_preset.lockup_enabled ==
                reference_defaults.lockup_enabled);
  ...
  {
    std::cout << "  Test 4: T300 specialized preset verification..."
              << std::endl;

    // ⚠️ IMPORTANT: T300 preset index depends on Config.cpp LoadPresets() order!
    // Current order: 0=Default, 1=T300, 2=GT3, 3=LMPx/HY, 4=GM, 5=GM+Yaw,
    // 6+=Test presets If presets are added/removed BEFORE T300 in Config.cpp,
    // update this index!
  ...
  // Test 5: Verify applying preset produces same result as
  // ApplyDefaultsToEngine()
  {
    ...
    ASSERT_TRUE(engine1.m_slip_angle_smoothing ==
                engine2.m_slip_angle_smoothing);
    ASSERT_TRUE(engine1.m_sop_smoothing_factor ==
                engine2.m_sop_smoothing_factor);
    ASSERT_TRUE(engine1.m_yaw_accel_smoothing == engine2.m_yaw_accel_smoothing);
    ASSERT_TRUE(engine1.m_chassis_inertia_smoothing ==
                engine2.m_chassis_inertia_smoothing);

    std::cout << "    Both initialization paths produce identical results"
              << std::endl;
  }
  ...
  std::cout << "  [SUMMARY] Single source of truth verified across all "
