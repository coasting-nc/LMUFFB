...
TEST_CASE_TAGGED(test_issue_265_metadata_sync, "Functional", (std::vector<std::string>{"logging", "metadata"})) {
    ...
    // Call UpdateMetadata
    engine.m_metadata.UpdateMetadata(data);

    // Verify engine state
    ASSERT_EQ_STR(engine.m_metadata.m_vehicle_name, "Ferrari 499P");
    ASSERT_EQ_STR(engine.m_metadata.m_track_name, "Le Mans");
    ASSERT_EQ_STR(engine.m_metadata.m_current_class_name, "Hypercar");
    ...
    ASSERT_EQ_STR(engine.m_metadata.m_vehicle_name, "Porsche 963");
    ASSERT_EQ_STR(engine.m_metadata.m_track_name, "Spa");
    ASSERT_EQ_STR(engine.m_metadata.m_current_class_name, "Hypercar");
    ...
    ASSERT_EQ_STR(engine.m_metadata.m_track_name, "Monza");
    ASSERT_EQ_STR(engine.m_metadata.m_vehicle_name, "Porsche 963");
}
