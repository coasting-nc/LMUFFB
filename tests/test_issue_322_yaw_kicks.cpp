...
TEST_CASE(test_unloaded_yaw_kick_activation, "YawKicks") {
    ...
    // force = -1.0 * 0.95 * 1.0 (gain) * 5.0 (BASE) * 0.5 (drop) = -2.375
    ASSERT_NEAR(yaw_force, -2.375f, 0.1);
}
...
TEST_CASE(test_power_yaw_kick_activation, "YawKicks") {
    ...
    // force = -1.0 * 0.95 * 1.0 (gain) * 5.0 (BASE) * 1.0 (vuln) = -4.75
    ASSERT_NEAR(yaw_force, -4.75f, 0.1);
}
...
TEST_CASE(test_yaw_jerk_punch, "YawKicks") {
    ...
    // Frame 2: Step to 1.0 rad/s^2. Jerk = (1 - 0) / 0.01 = 100 rad/s^3
    // Punchy Accel = 1.0 + (100 * 0.1) = 11.0
    data.mLocalRot.y = 0.01;
    data.mElapsedTime = 1.01;
    engine.calculate_force(&data, "GT3", "TestCar", 0.0f, true, 0.01);
    ...
    // force = -1.0 * 11.0 * 1.0 * 5.0 = -55.0.
    // Wait, 11.0 is normalized against 10.0 for gamma.
    // If gamma is 1.0, and yaw_norm = clamp(11.0/10.0, 0, 1) = 1.0.
    // shaped = 1.0 * 10.0 = 10.0.
    // force = -1.0 * 10.0 * 1.0 * 5.0 = -50.0.
    ASSERT_NEAR(force_with_punch, -50.0f, 1.0);
}
...
TEST_CASE(test_yaw_kick_blending, "YawKicks") {
    ...
    // General: -1.0 * 0.5 * 5.0 = -2.5
    // Unloaded: -1.0 * 1.0 * 5.0 * 0.8 (drop) = -4.0
    // Power: -1.0 * 2.0 * 5.0 * 1.0 (slip) = -10.0
    // Max abs is Power: -10.0
    ASSERT_NEAR(final_force, -10.0f, 0.1);
}
