...
/**
 * Regression test for GameConnector robust state machine and "zombie connection" fix.
...
TEST_CASE(test_game_connector_robust_logic, "System") {
#ifdef _WIN32
    std::cout << "  [INFO] Running GameConnector logic test with simulated Windows Shared Memory." << std::endl;
    ...
    bool connected = GameConnector::Get().TryConnect();
    ASSERT_TRUE(connected);
    // IsConnected() should be FALSE because window 9999 is invalid
    ASSERT_FALSE(GameConnector::Get().IsConnected());
    ...
    ASSERT_TRUE(try_res);
    ASSERT_TRUE(GameConnector::Get().IsConnected());
    ...
    bool copy_res = GameConnector::Get().CopyTelemetry(dest);
    
    if (!copy_res) {
        std::cout << "  [FAIL_DIAG] copy_res=false. Conn=" << GameConnector::Get().IsConnected() << std::endl;
    }
    ...
    // Verify that GameConnector forgot the track name (Internal state reset)
    ...
    // Ensure GameConnector is clean for other tests
    GameConnector::Get().Disconnect();
