...
#define ASSERT_TRUE(condition)                                                 \
  if (condition) {                                                             \
    ...
    std::cout << "[FAIL] " << #condition << " (" << __FILE__ << ":"            \
              << __LINE__ << ")" << std::endl;                                 \
    g_tests_failed++;                                                          \
  }
...
// Filter Settling Period: Number of frames needed for smoothing filters to
// converge Used throughout tests to ensure stable state before assertions
...
static void
test_regression_yaw_slide_feedback(); // Forward declaration (v0.4.18)
static void
test_yaw_kick_signal_conditioning();         // Forward declaration (v0.4.42)
static void test_coordinate_sop_inversion(); // Forward declaration (v0.4.19)
static void
test_coordinate_rear_torque_inversion(); // Forward declaration (v0.4.19)
static void
test_coordinate_scrub_drag_direction(); // Forward declaration (v0.4.19)
static void
test_coordinate_debug_slip_angle_sign(); // Forward declaration (v0.4.19)
static void
test_regression_no_positive_feedback(); // Forward declaration (v0.4.19)
static void
test_coordinate_all_effects_alignment(); // Forward declaration (v0.4.21)
static void
test_regression_phase_explosion();           // Forward declaration (Regression)
...
static void
test_chassis_inertia_smoothing_convergence();  // Forward declaration (v0.4.39)
...
static void
test_config_safety_validation_v057();           // Forward declaration (v0.5.7)
...
static void
test_notch_filter_edge_cases(); // Forward declaration (v0.6.10 - Edge Cases)
static void
test_yaw_kick_edge_cases(); // Forward declaration (v0.6.10 - Edge Cases)
 ...
 */
static TelemInfoV01 CreateBasicTestTelemetry(double speed = 20.0,
  ...
  for (int i = 0; i < 4; i++) {
    ...
    data.mWheel[i].mVerticalTireDeflection =
        0.001; // Avoid "missing data" warning (v0.6.21)
  }
 ...
 * Required after v0.5.12 refactoring removed default initializers from
 * FFBEngine.h.
 *
 * Note: Returns a reference to avoid copy (FFBEngine has deleted copy
 * constructor). Each call reinitializes the same static instance.
 *
...
static void InitializeEngine(FFBEngine &engine) {
  ...
  // v0.6.31: Zero out all auxiliary effects for clean physics testing by
  // default. Individual tests can re-enable what they need.
  //
  // IMPORTANT FOR TEST AUTHORS (v0.6.31):
  // This is a BREAKING CHANGE from previous test behavior. Before v0.6.31,
  // tests inherited default values from Preset struct (e.g., m_sop_effect
  // = 1.5, m_understeer_effect = 1.0). Now, InitializeEngine() explicitly zeros
  // all effects to ensure test isolation.
  //
  // If your test needs a specific effect enabled, you MUST explicitly set it
  // after calling InitializeEngine(). Do not rely on default values. This
  // prevents cross-contamination between tests and makes test intent explicit.
  ...
  // v0.6.25: Disable speed gate by default for legacy tests (avoids muting
  // physics at 0 speed)
...
static void test_high_gain_stability() {
  ...
  TelemInfoV01 data =
      CreateBasicTestTelemetry(20.0, 0.15); // Sliding mid-corner
  ...
  std::cout << "[PASS] Engine stable at 200% Gain and 10.0 ABS Gain."
            << std::endl;
  g_tests_passed++;
}
...
static void test_lockup_pitch_scaling() {
  ...
  data.mWheel[0].mLongitudinalPatchVel =
      -5.0; // Trigger lockup (approx -25% slip)
...
static void test_base_force_modes() {
  ...
  if (std::abs(force_native - 0.25) < 0.001) {
    std::cout << "[PASS] Native Mode: Correctly attenuated (0.25)."
              << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Native Mode: Got " << force_native << " Expected 0.25."
              << std::endl;
    g_tests_failed++;
  }
  ...
  if (std::abs(force_synthetic - 0.5) < 0.001) {
    std::cout << "[PASS] Synthetic Mode: Constant force applied (0.5)."
              << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Synthetic Mode: Got " << force_synthetic
  ...
  if (std::abs(force_muted) < 0.001) {
    ...
    std::cout << "[FAIL] Muted Mode: Got " << force_muted << " Expected 0.0."
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_sop_yaw_kick() {
  ...
  engine.m_yaw_accel_smoothing =
      0.0225f; // v0.5.8: Explicitly set legacy value for test expectations
  ...
  // v0.4.20 UPDATE: With force inversion, first frame should be ~-0.025 (10% of
  // steady-state due to LPF) The negative sign is correct - provides
  // counter-steering cue
  if (std::abs(force - (-0.025)) < 0.005) {
    std::cout << "[PASS] Yaw Kick first frame smoothed correctly (" << force
              << " ≈ -0.025)." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Yaw Kick first frame mismatch. Got " << force
              << " Expected ~-0.025." << std::endl;
    g_tests_failed++;
  }
}
...
static void test_scrub_drag_fade() {
  ...
  if (std::abs(std::abs(force) - 0.125) < 0.001) {
    ...
    std::cout << "[FAIL] Scrub drag fade incorrect. Got " << force
              << " Expected 0.125." << std::endl;
    g_tests_failed++;
  }
}
...
static void test_road_texture_teleport() {
  ...
  if (std::abs(force - 0.05) < 0.001) {
    ...
    std::cout << "[FAIL] Teleport spike unclamped? Got " << force
...
static void test_grip_low_speed() {
  ...
  if (std::abs(force - 1.0) < 0.001) {
    ...
    std::cout << "[FAIL] Low speed grip not forced. Got " << force
...
static void test_zero_input() {
  ...
  // v0.4.5: Set Ride Height > 0.002 to avoid Scraping effect (since memset 0
  // implies grounded)
  data.mWheel[0].mRideHeight = 0.1;
  data.mWheel[1].mRideHeight = 0.1;

  // Set some default load to avoid triggering sanity check defaults if we want
  // to test pure zero input? Actually, zero input SHOULD trigger sanity checks
  // now.
...
static void test_min_force() {
  ...
  // 20.0 is Max. Min force 0.10 means we want at least 2.0 Nm output
  // effectively. Input 0.05 Nm. 0.05 / 20.0 = 0.0025.
  ...
  if (std::abs(force - 0.10) > 0.001) {
    std::cout << "Debug Min Force: Calculated " << force << " Expected 0.10"
              << std::endl;
  }
...
static void test_progressive_lockup() {
  ...
  // Debug
  // std::cout << "Force Low: " << force_low << " Phase: " <<
  // engine.m_lockup_phase << std::endl;
...
static void test_slide_texture() {
  ...
  // v0.4.39 UPDATE: Work-Based Scrubbing requires grip LOSS to generate
  // vibration Gripping tires (grip=1.0) should NOT scrub, even with high
  // lateral velocity
  {
    FFBEngine engine;
    InitializeEngine(engine); // v0.5.12: Initialize with T300 defaults
    ...
    data.mDeltaTime =
        0.013; // 13ms. For 35Hz (5m/s input), period is 28ms.
    ...
    if (std::abs(force) > 0.001) {
      std::cout << "[PASS] Front slip triggers Slide Texture (Force: " << force
                << ")" << std::endl;
      g_tests_passed++;
    } else {
      std::cout << "[FAIL] Front slip failed to trigger Slide Texture."
                << std::endl;
      g_tests_failed++;
    }
  }
  ...
  // Case 2: Rear Slip (Oversteer/Drift)
  {
    ...
    data.mWheel[0].mGripFract =
        0.5; // Simulate front grip loss to enable global slide effect
    data.mWheel[1].mGripFract = 0.5;
    data.mWheel[0].mTireLoad =
        4000.0; // Front Load required for effect amplitude scaling
    data.mWheel[1].mTireLoad = 4000.0;
    ...
    if (std::abs(force) > 0.001) {
      std::cout << "[PASS] Rear slip triggers Slide Texture (Force: " << force
                << ")" << std::endl;
      g_tests_passed++;
    } else {
      std::cout << "[FAIL] Rear slip failed to trigger Slide Texture."
                << std::endl;
      g_tests_failed++;
    }
  }
...
static void test_suspension_bottoming() {
  ...
  if (std::abs(force_f1) > 0.0001) {
    std::cout << "[PASS] Bottoming effect active. Force: " << force_f1
              << std::endl;
    g_tests_passed++;
  } else {
...
static void test_phase_wraparound() {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    if (engine.m_lockup_phase < prev_phase) {
      ...
      // We check it's at least > 3.0 to ensure it's not resetting randomly at
      // 0.
      if (!(prev_phase > 3.0)) {
        std::cout << "[FAIL] Wrapped phase too early: " << prev_phase
                  << std::endl;
        g_tests_failed++;
      }
    }
  ...
  if (wrap_count > 0) {
    std::cout << "[PASS] Phase wrapped " << wrap_count
              << " times without discontinuity." << std::endl;
    g_tests_passed++;
  } else {
...
static void test_multi_effect_interaction() {
  ...
  // lockup phase (40Hz at 20m/s) wraps exactly to 0 after 10 frames with
  // dt=0.01.
  for (int i = 0; i < 11; i++) {
  ...
  if (lockup_ok && spin_ok) {
    ...
    if (std::abs(engine.m_lockup_phase - engine.m_spin_phase) > 0.1) {
      std::cout << "[PASS] Multiple effects coexist without interference."
                << std::endl;
      g_tests_passed++;
    } else {
    ...
    std::cout << "[FAIL] Effects did not trigger. lockup_phase="
              << engine.m_lockup_phase << ", spin_phase=" << engine.m_spin_phase
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_load_factor_edge_cases() {
  ...
  // NOTE: This test will fail until we tune down the texture gains for Nm
  // scale. But structurally it passes compilation.

  if (std::abs(force_extreme) < 0.15) {
    std::cout << "[PASS] Load factor clamped correctly." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Load factor not clamped? Force: " << force_extreme
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_spin_torque_drop_interaction() {
  ...
  if (std::abs(force_with_spin - force_no_spin) > 0.02) {
    ...
    std::cout << "[FAIL] Torque drop ineffective. Spin: " << force_with_spin
              << " NoSpin: " << force_no_spin << std::endl;
    g_tests_failed++;
  }
}
...
static void test_rear_grip_fallback() {
  ...
  data.mWheel[1].mGripFract = 1.0;
  data.mWheel[0].mTireLoad =
      4000.0; // Ensure Front Load > 100 for fallback trigger
  data.mWheel[1].mTireLoad = 4000.0;
  ...
  // If fallback fails, it uses 0.0 -> Grip Delta = 1.0 - 0.0 = 1.0 -> Massive
  ...
  // engine doesn't expose avg_rear_grip publically, but we can infer from
  // Lateral G Boost (Slide). If grip restored to 1.0, delta = 1.0 - 1.0 = 0.0.
  // No boost. If grip is 0.0, delta = 1.0. Boost applied.
  ...
  if (!batch.empty()) {
    ...
    if (std::abs(boost) < 0.001) {
      std::cout << "[PASS] Lateral G Boost (Slide) correctly suppressed (Rear "
                   "Grip restored)."
                << std::endl;
      g_tests_passed++;
    } else {
      std::cout << "[FAIL] False Lateral G Boost (Slide) detected: " << boost
                << std::endl;
      g_tests_failed++;
    }
  } else {
...
static void test_sanity_checks() {
  ...
  if (std::abs(force_corrected) > 0.001) {
    std::cout << "[PASS] Load fallback applied (Force generated: "
              << force_corrected << ")" << std::endl;
    g_tests_passed++;
  } else {
  ...
  //
  // TEST PURPOSE: Verify that the engine detects missing grip telemetry and
  // applies the slip angle-based approximation fallback mechanism.
  //
  // SETUP:
  ...
  engine.m_gain = 1.0;
  data.mSteeringShaftTorque =
      10.0; // 10 / 20.0 = 0.5 normalized (if grip = 1.0)

  // EXPECTED CALCULATION (see detailed notes above):
  // If grip is 0, grip_factor = 1.0 - ((1.0 - 0.0) * 1.0) = 0.0. Output force =
  // 0. If grip corrected to 0.2 (floor), grip_factor = 1.0 - ((1.0 - 0.2)
  // * 1.0) = 0.2. Output force = 2.0. Norm force = 2.0 / 20.0 = 0.1.
  ...
  ASSERT_NEAR(force_grip, 0.1, 0.001); // Expect minimum grip correction (0.2
                                       // grip -> 0.1 normalized force)
...
static void test_hysteresis_logic() {
  ...
  if (engine.m_missing_load_frames == 5) {
    ...
    std::cout << "[FAIL] Hysteresis counter not 5: "
              << engine.m_missing_load_frames << std::endl;
    g_tests_failed++;
  }
  ...
  if (engine.m_warned_load) {
    std::cout << "[PASS] Hysteresis triggered fallback (Warning set)."
              << std::endl;
    g_tests_passed++;
  } else {
  ...
  if (engine.m_missing_load_frames < 25) {
    std::cout << "[PASS] Hysteresis counter decrementing on recovery."
              << std::endl;
    g_tests_passed++;
  }
}
...
static void test_presets() {
  ...
  // Verify
  // Update expectation: Test: SoP Only uses default 1.0f Gain in Config.cpp
  // (not 0.5f)
  ...
  if (gain_ok && sop_ok && under_ok) {
    std::cout << "[PASS] Preset applied correctly (Gain=" << engine.m_gain
              << ", SoP=" << engine.m_sop_effect << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Preset mismatch. Gain: " << engine.m_gain
              << " SoP: " << engine.m_sop_effect << std::endl;
    g_tests_failed++;
  }
}
...
static void test_config_persistence() {
  ...
  engine_save.m_road_texture_gain =
      1.5f; // v0.4.50: Use value within safe range (max 2.0)
...
static void test_smoothing_step_response() {
  ...
  if (force1 > 0.0 && force1 < 0.005) {
    std::cout << "[PASS] Smoothing Step 1 correct (" << force1
              << ", small positive)." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Smoothing Step 1 mismatch. Got " << force1
              << std::endl;
    g_tests_failed++;
  }
  ...
  if (force1 > 0.02 && force1 < 0.06) {
    std::cout << "[PASS] Smoothing settled to steady-state (" << force1
              << ", near 0.05)." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Smoothing did not settle. Value: " << force1
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_universal_bottoming() {
  ...
  if (std::abs(force_scrape) > 0.001) {
    std::cout << "[PASS] Bottoming Method A (Scrape) Triggered. Force: "
              << force_scrape << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Bottoming Method A Failed. Force: " << force_scrape
              << std::endl;
    g_tests_failed++;
  }
  ...
  // Frame 2: Massive Spike (e.g. +5000N in 0.005s -> 1,000,000 N/s > 100,000
  // threshold)
  ...
  if (std::abs(force_spike) > 0.001) {
    std::cout << "[PASS] Bottoming Method B (Spike) Triggered. Force: "
              << force_spike << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Bottoming Method B Failed. Force: " << force_spike
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_preset_initialization() {
  ...
  // REGRESSION TEST: Verify all built-in presets properly initialize tuning
  // fields. v0.6.30: T300 preset is now specialized with optimized values.

  Config::LoadPresets();

  // ⚠️ IMPORTANT: These expected values MUST match Config.h default member
  // initializers! When changing the default preset in Config.h, update these
  // values to match. Also update SetAdvancedBraking() default parameters in
  // Config.h. See Config.h line ~12 for the single source of truth.
  //
  // Expected default values for generic presets (updated to GT3 defaults in
  // v0.6.35)
  const float expected_abs_freq = 25.5f; // Changed from 20.0 to match GT3
  const float expected_lockup_freq_scale =
      1.02f; // Changed from 1.0 to match GT3
  ...
  // ⚠️ IMPORTANT: This array MUST match the exact order of presets in Config.cpp
  // LoadPresets()! When adding/removing/reordering presets in Config.cpp,
  // update this array AND the loop count below. Current count: 14 presets
  // (v0.6.35: Added 4 DD presets after T300)
  const char *preset_names[] = {"Default",
                                "T300",
  ...
  for (int i = 0; i < 14; i++) {
    if (i >= Config::presets.size()) {
      std::cout << "[FAIL] Preset " << i << " (" << preset_names[i]
                << ") not found!" << std::endl;
      all_passed = false;
      continue;
    }
    ...
    if (preset.name != preset_names[i]) {
      std::cout << "[FAIL] Preset " << i << " name mismatch: expected '"
                << preset_names[i] << "', got '" << preset.name << "'"
                << std::endl;
      all_passed = false;
      continue;
    }
    ...
    // Specialized presets have custom-tuned values that differ from Config.h
    // defaults. They should NOT be validated against expected_abs_freq,
    // expected_lockup_freq_scale, etc.
    //
    // ⚠️ IMPORTANT: When adding new specialized presets to Config.cpp, add them
    // to this list! Current specialized presets: Default, T300, GT3, LMPx/HY,
    // GM, GM + Yaw Kick
    bool is_specialized =
        (preset.name == "Default" || preset.name == "T300" ||
    ...
    if (!is_specialized) {
      ...
      if (std::abs(preset.lockup_freq_scale - exp_lockup_f) > 0.001f) {
        std::cout << "[FAIL] " << preset.name
                  << ": lockup_freq_scale = " << preset.lockup_freq_scale
                  << ", expected " << exp_lockup_f << std::endl;
        fields_ok = false;
      }

      if (std::abs(preset.scrub_drag_gain - exp_scrub) > 0.001f) {
        std::cout << "[FAIL] " << preset.name
                  << ": scrub_drag_gain = " << preset.scrub_drag_gain
                  << ", expected " << exp_scrub << std::endl;
        fields_ok = false;
      }
      ...
      if (preset.abs_freq != expected_abs_freq) {
        std::cout << "[FAIL] " << preset.name
                  << ": abs_freq = " << preset.abs_freq << ", expected "
                  << expected_abs_freq << std::endl;
        fields_ok = false;
      }

      if (preset.spin_freq_scale != expected_spin_freq_scale) {
        std::cout << "[FAIL] " << preset.name
                  << ": spin_freq_scale = " << preset.spin_freq_scale
                  << ", expected " << expected_spin_freq_scale << std::endl;
        fields_ok = false;
      }

      if (preset.bottoming_method != expected_bottoming_method) {
        std::cout << "[FAIL] " << preset.name
                  << ": bottoming_method = " << preset.bottoming_method
                  << ", expected " << expected_bottoming_method << std::endl;
        fields_ok = false;
      }
    }
    ...
    if (is_specialized_t300) {
      if (std::abs(preset.understeer - t300_understeer) > 0.001f) {
        std::cout << "[FAIL] T300: Optimized understeer (" << preset.understeer
                  << ") != " << t300_understeer << std::endl;
        fields_ok = false;
      }
      if (std::abs(preset.sop - t300_sop) > 0.001f) {
        std::cout << "[FAIL] T300: Optimized SoP (" << preset.sop
                  << ") != " << t300_sop << std::endl;
        fields_ok = false;
      }
      if (preset.steering_shaft_smoothing != t300_shaft_smooth) {
        std::cout << "[FAIL] T300: Optimized shaft smoothing ("
                  << preset.steering_shaft_smoothing
                  << ") != " << t300_shaft_smooth << std::endl;
        fields_ok = false;
      }
      if (preset.notch_q != t300_notch_q) {
        std::cout << "[FAIL] T300: Optimized notch_q (" << preset.notch_q
                  << ") != " << t300_notch_q << std::endl;
        fields_ok = false;
      }
    }

    if (fields_ok) {
      std::cout << "[PASS] " << preset.name << ": fields verified correctly"
                << (is_specialized_t300 ? " (Including v0.6.30 optimizations)"
                                        : "")
                << std::endl;
      g_tests_passed++;
    } else {
  ...
  if (all_passed) {
    ...
    std::cout << "[FAIL] Some presets have incorrect specialization or defaults"
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_regression_road_texture_toggle() {
  ...
  if (std::abs(force) < 0.01) {
    ...
    std::cout << "[FAIL] Spike detected! State was stale. Force: " << force
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_regression_bottoming_switch() {
  ...
  engine.calculate_force(
      &data); // Should update m_prev_susp_force even if Method A is active
  ...
  // If broken: dForce = (5000 - 0) / dt = 500,000. Massive spike triggers
  // effect.
  ...
  if (std::abs(force) < 0.001) {
    ...
    std::cout << "[FAIL] Spike detected on switch! Force: " << force
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_regression_rear_torque_lpf() {
  ...
  // This means Rear Torque is 0.0 (because grip is good), BUT LPF should be
  // tracking the slide.
  ...
  if (force < -0.25) { // v0.4.19: Expect NEGATIVE force (counter-steering)
    std::cout << "[PASS] LPF was running in background. Force: " << force
              << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] LPF was stale/reset. Force too low: " << force
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_stress_stability() {
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    if (std::isnan(force) || std::isinf(force)) {
      std::cout << "[FAIL] Iteration " << i << " produced NaN/Inf!"
                << std::endl;
      failed = true;
      break;
    }
    ...
    if (force > 1.00001 || force < -1.00001) {
      std::cout << "[FAIL] Iteration " << i << " exceeded bounds: " << force
                << std::endl;
      failed = true;
      break;
    }
  ...
  if (!failed) {
    std::cout << "[PASS] Survived 1000 iterations of random input."
              << std::endl;
    g_tests_passed++;
  } else {
    g_tests_failed++;
...
static void test_yaw_accel_smoothing() {
  ...
  // Without smoothing, this would be -10.0 * 1.0 * 5.0 / 20.0 = -2.5 (clamped
  // to -1.0) With smoothing (alpha=0.1), first frame = -0.25
  if (std::abs(force_frame1 - (-0.25)) < 0.01) {
    std::cout << "[PASS] First frame smoothed to 10% of raw input ("
              << force_frame1 << " ~= -0.25)." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] First frame smoothing incorrect. Got " << force_frame1
              << " Expected ~-0.25." << std::endl;
    g_tests_failed++;
  }

  // v0.4.20 UPDATE: With force inversion, values are negative
  // Smoothed (frame 2): -1.0 + 0.1 * (-10.0 - (-1.0)) = -1.0 + 0.1 * (-9.0) =
  // -1.9 Force: -1.9 * 1.0 * 5.0 = -9.5 Nm Normalized: -9.5 / 20.0 = -0.475
  double force_frame2 = engine.calculate_force(&data);

  if (std::abs(force_frame2 - (-0.475)) < 0.02) {
    std::cout << "[PASS] Second frame accumulated correctly (" << force_frame2
              << " ~= -0.475)." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Second frame accumulation incorrect. Got "
              << force_frame2 << " Expected ~-0.475." << std::endl;
    g_tests_failed++;
  }
  ...
  // With smoothing, the max force should be much smaller than the raw input
  // would produce Raw would give: 5.0 * 1.0 * 5.0 / 20.0 = 1.25 (clamped
  // to 1.0) Smoothed should stay well below 0.5
  if (max_force < 0.5) {
    std::cout << "[PASS] High-frequency noise rejected (max force " << max_force
              << " < 0.5)." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] High-frequency noise not rejected. Max force: "
              << max_force << std::endl;
    g_tests_failed++;
  }
}
...
static void test_yaw_accel_convergence() {
  ...
  // Expected steady-state: -1.0 * 1.0 * 5.0 / 20.0 = -0.25
  // After 50 frames with alpha=0.1, should be very close to steady-state
  // (-0.25) Formula: smoothed = target * (1 - (1-alpha)^n) After 50 frames:
  // smoothed ~= -1.0 * (1 - 0.9^50) ~= -0.9948 Force: -0.9948 * 1.0 * 5.0
  // / 20.0 ~= -0.2487
  if (std::abs(force - (-0.25)) < 0.01) {
    std::cout << "[PASS] Converged to steady-state after 50 frames (" << force
              << " ~= -0.25)." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Did not converge. Got " << force << " Expected ~-0.25."
              << std::endl;
    g_tests_failed++;
  }
  ...
  // If prev_smoothed ~= -0.9948, new = -0.9948 + 0.1 * (0.0 - (-0.9948)) =
  // -0.8953 Force: -0.8953 * 1.0 * 5.0 / 20.0 ~= -0.224
  if (force_after_change > force && force_after_change < -0.2) {
    std::cout << "[PASS] Smoothly decaying after step change ("
              << force_after_change << ")." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Decay behavior incorrect. Got " << force_after_change
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_regression_yaw_slide_feedback() {
  ...
  // Simulate high-frequency yaw acceleration noise (what Slide Rumble would
  // cause) Alternate between +10 and -10 rad/s^2 (extreme noise)
  double max_force = 0.0;
  ...
  // Without smoothing (v0.4.16), this would create a feedback loop with forces
  // > 1.0 With smoothing (v0.4.18), max force should stay reasonable (< 1.0,
  // ideally < 0.8)
  if (max_force < 1.0) {
    std::cout << "[PASS] No feedback loop detected (max force " << max_force
              << " < 1.0)." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Potential feedback loop! Max force: " << max_force
              << std::endl;
    g_tests_failed++;
  }
  ...
  if (avg_force < 0.5) {
    std::cout << "[PASS] Average force remains low (avg " << avg_force
              << " < 0.5)." << std::endl;
    g_tests_passed++;
  } else {
  ...
  if (std::abs(final_force) < 0.1) {
    std::cout << "[PASS] System settled after noise removed (final force "
              << final_force << ")." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] System did not settle. Final force: " << final_force
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_yaw_kick_signal_conditioning() {
  ...
  engine.m_gyro_gain = 0.0f;
  engine.m_invert_force = false;
  engine.m_yaw_kick_threshold = 0.2f; // Explicitly set threshold for this test
                                      // (v0.6.35: Don't rely on defaults)
  ...
  std::cout << "  Case 1: Idle Noise (YawAccel = 0.1, below threshold)"
            << std::endl;
  ...
  if (std::abs(force_idle) < 0.01) {
    std::cout << "[PASS] Idle noise filtered (force = " << force_idle
              << " ~= 0.0)." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Idle noise not filtered. Got " << force_idle
              << " Expected ~0.0." << std::endl;
    g_tests_failed++;
  }

  // Test Case 2: Low Speed Cutoff
  std::cout << "  Case 2: Low Speed (YawAccel = 5.0, Speed = 1.0 m/s)"
            << std::endl;
  ...
  if (std::abs(force_low_speed) < 0.01) {
    std::cout << "[PASS] Low speed cutoff active (force = " << force_low_speed
              << " ~= 0.0)." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Low speed cutoff failed. Got " << force_low_speed
              << " Expected ~0.0." << std::endl;
    g_tests_failed++;
  }

  // Test Case 3: Valid Kick - High Speed + High Yaw Accel
  std::cout << "  Case 3: Valid Kick (YawAccel = 5.0, Speed = 20.0 m/s)"
            << std::endl;
  engine.m_yaw_accel_smoothed = 0.0; // Reset smoothed state
  ...
  if (force_valid < -0.1) {
    std::cout << "[PASS] Valid kick detected (force = " << force_valid << ")."
              << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Valid kick not detected correctly. Got " << force_valid
              << "." << std::endl;
    g_tests_failed++;
  }
}
...
static void test_notch_filter_attenuation() {
  ...
  for (int i = 0; i < 400; i++) {
    ...
    if (i > 100 && std::abs(out) > max_amp_target)
      max_amp_target = std::abs(out);
  }

  if (max_amp_target < 0.1) {
    std::cout << "[PASS] Notch Filter attenuated target frequency (Max Amp: "
              << max_amp_target << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout
        << "[FAIL] Notch Filter did not attenuate target frequency. Max Amp: "
        << max_amp_target << std::endl;
    g_tests_failed++;
  }
  ...
  for (int i = 0; i < 400; i++) {
    ...
    double out = filter.Process(in);
    if (i > 100 && std::abs(out) > max_amp_pass)
      max_amp_pass = std::abs(out);
  }

  if (max_amp_pass > 0.8) {
    std::cout << "[PASS] Notch Filter passed off-target frequency (Max Amp: "
              << max_amp_pass << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout
        << "[FAIL] Notch Filter attenuated off-target frequency. Max Amp: "
        << max_amp_pass << std::endl;
    g_tests_failed++;
  }
}
...
static void test_frequency_estimator() {
  ...
  for (int i = 0; i < 400; i++) {
    ...
    data.mSteeringShaftTorque =
        5.0 * std::sin(2.0 * 3.14159265 * target_freq * t);
    data.mElapsedTime = t;
  ...
  if (std::abs(estimated - target_freq) < 1.0) {
    std::cout << "[PASS] Frequency Estimator converged to " << estimated
              << " Hz (Target: " << target_freq << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Frequency Estimator mismatch. Got " << estimated
              << " Hz, Expected ~" << target_freq << std::endl;
    g_tests_failed++;
  }
}
...
static void test_snapshot_data_integrity() {
  ...
  if (!batch_load.empty()) {
    ...
    if (std::abs(snap_load.raw_front_tire_load) < 0.001) {
      ...
      std::cout << "[FAIL] Raw Front Tire Load incorrect: "
                << snap_load.raw_front_tire_load << std::endl;
      g_tests_failed++;
    }

    // Test 2: Calculated Load should be approx 1300 (SuspForce 1000 + 300
    // offset)
    if (std::abs(snap_load.calc_front_load - 1300.0) < 0.001) {
      std::cout << "[PASS] Calculated Front Load is 1300.0." << std::endl;
      g_tests_passed++;
    } else {
      std::cout << "[FAIL] Calculated Front Load incorrect: "
                << snap_load.calc_front_load << std::endl;
      g_tests_failed++;
    }

    // Test 3: Raw Throttle Input (from initial setup: data.mUnfilteredThrottle
    // = 0.8)
    if (std::abs(snap_load.raw_input_throttle - 0.8) < 0.001) {
      std::cout << "[PASS] Raw Throttle captured." << std::endl;
      g_tests_passed++;
    } else {
      std::cout << "[FAIL] Raw Throttle incorrect: "
                << snap_load.raw_input_throttle << std::endl;
      g_tests_failed++;
    }
    ...
    if (std::abs(snap_load.raw_front_ride_height - 0.03) < 0.001) {
      ...
      std::cout << "[FAIL] Raw Ride Height incorrect: "
                << snap_load.raw_front_ride_height << std::endl;
      g_tests_failed++;
    }
  }
  ...
  if (std::abs(snap.calc_front_grip - 1.0) < 0.001) {
    ...
    std::cout << "[FAIL] Calc Front Grip incorrect: " << snap.calc_front_grip
              << std::endl;
    g_tests_failed++;
  }
  ...
  if (std::abs(snap.calc_rear_grip - 0.5) < 0.001) {
    ...
    std::cout << "[FAIL] Calc Rear Grip incorrect: " << snap.calc_rear_grip
              << std::endl;
    g_tests_failed++;
  }
  ...
  if (std::abs(snap.raw_front_deflection - 0.05) < 0.001) {
    std::cout << "[PASS] raw_front_deflection captured (Renamed field)."
              << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] raw_front_deflection incorrect: "
              << snap.raw_front_deflection << std::endl;
    g_tests_failed++;
  }
}
...
static void test_zero_effects_leakage() {
  ...
  data.mWheel[2].mLateralForce =
      0.0; // Simulate missing force (workaround trigger)
...
static void test_snapshot_data_v049() {
  ...
  if (std::abs(snap.raw_front_long_patch_vel - 1.0) < 0.001) {
    ...
    std::cout << "[FAIL] raw_front_long_patch_vel: "
              << snap.raw_front_long_patch_vel << std::endl;
    g_tests_failed++;
  }
  ...
  if (std::abs(snap.raw_rear_lat_patch_vel - 2.0) < 0.001) {
    ...
    std::cout << "[FAIL] raw_rear_lat_patch_vel: "
              << snap.raw_rear_lat_patch_vel << std::endl;
    g_tests_failed++;
  }
  ...
  if (std::abs(snap.raw_rear_long_patch_vel - 3.0) < 0.001) {
    ...
    std::cout << "[FAIL] raw_rear_long_patch_vel: "
              << snap.raw_rear_long_patch_vel << std::endl;
    g_tests_failed++;
  }
  ...
  if (std::abs(snap.raw_rear_slip_angle - 0.0996) < 0.01) {
    ...
    std::cout << "[FAIL] raw_rear_slip_angle: " << snap.raw_rear_slip_angle
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_rear_force_workaround() {
  ...
  // 3. Create a realistic slip angle scenario (5 m/s lateral, 20 m/s
  // longitudinal)
  ...
  engine.m_oversteer_boost =
      1.0; // Enable Lateral G Boost (Slide) (multiplies rear torque)
  engine.m_gain = 1.0;       // Full gain
  engine.m_sop_scale = 10.0; // Moderate SoP scaling
  engine.m_rear_align_effect =
      1.0f; // Fix effect gain for test calculation (Default is now 5.0)
  engine.m_invert_force = false; // Ensure non-inverted for formula check
  engine.m_max_torque_ref =
      100.0f; // Explicitly use 100 Nm ref for snapshot scaling (v0.4.50)
  ...
  // Set TireLoad to 0 to prove we don't use it (API bug often kills both
  // fields)
  ...
  // With Lat = 5 m/s, Long = 20 m/s: atan(5/20) = atan(0.25) ≈ 0.2449 rad ≈ 14
  // degrees This represents a moderate cornering scenario.
  data.mWheel[2].mLateralPatchVel = 5.0;
  data.mWheel[3].mLateralPatchVel = 5.0;
  data.mWheel[2].mLongitudinalGroundVel = 20.0;
  data.mWheel[3].mLongitudinalGroundVel = 20.0;
  ...
  // On the first frame, the LPF formula is: smoothed = prev + alpha × (raw -
  // prev) With prev = 0 (initial state) and alpha ≈ 0.1:
  ...
  // Rear torque should be NEGATIVE for counter-steering (pulling left for a
  // right slide) So expected torque is -1.21 Nm v0.4.37 Update: Time-Corrected
  // Smoothing (tau=0.0225) with dt=0.01 (100Hz), alpha = 0.01 / (0.0225 + 0.01)
  // = 0.307 Expected = Raw (-12.13) * 0.307 = -3.73 Nm v0.4.40 Update: Reduced
  // tau to 0.015 for lower latency with dt=0.01 (100Hz), alpha = 0.01 / (0.015
  // + 0.01) = 0.4 Expected = Raw (-12.13) * 0.4 = -4.85 Nm v0.4.50 Update: FFB
  // snapshot now scales with MaxTorqueRef (Decoupling) with Ref=100.0, scale
  // = 5.0. Expected = -4.85 * 5.0 = -24.25 Nm
  double expected_torque =
      -24.25;                    // First-frame value with Decoupling (v0.4.50)
  ...
  if (rear_torque_nm > (expected_torque - torque_tolerance) &&
    ...
    std::cout << "[PASS] Rear torque snapshot correct (" << rear_torque_nm
              << " Nm, counter-steering)." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Rear torque outside expected range. Value: "
              << rear_torque_nm << " Nm (expected ~" << expected_torque
              << " Nm +/-" << torque_tolerance << ")" << std::endl;
    g_tests_failed++;
  }
}
...
static void test_rear_align_effect() {
  ...
  data.mWheel[0].mTireLoad = 4000.0;
  data.mWheel[1].mTireLoad = 4000.0; // Fronts valid
  data.mWheel[0].mGripFract = 1.0;
  data.mWheel[1].mGripFract = 1.0;
  ...
  if (force > (expected - tolerance) && force < (expected + tolerance)) {
    std::cout << "[PASS] Rear Force Workaround active. Value: " << force
              << " Nm" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Rear Force Workaround failed. Value: " << force
  ...
  if (!batch.empty()) {
    ...
    // v0.4.40: Updated to -3.46 Nm (tau=0.015, alpha=0.4, with 2x
    // rear_align_effect) v0.4.50: Decoupling (Ref=100) scales by 5.0. Expected
    // = -3.46 * 5.0 = -17.3 Nm
    ...
    if (rear_torque_nm > (expected_torque - torque_tolerance) &&
        rear_torque_nm < (expected_torque + torque_tolerance)) {
      std::cout << "[PASS] Rear Align Effect active and decoupled (Boost 0.0). "
                   "Value: "
                << rear_torque_nm << std::endl;
      g_tests_passed++;
    } else {
      std::cout << "[FAIL] Rear Align Effect failed. Value: " << rear_torque_nm
...
static void test_sop_yaw_kick_direction() {
  ...
  if (force < -0.05) { // Expect Negative (adjusted threshold for smoothed
                       // first-frame value)
    std::cout << "[PASS] Yaw Kick provides counter-steer (Negative Force: "
              << force << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Yaw Kick direction wrong. Got: " << force
              << " Expected Negative." << std::endl;
    g_tests_failed++;
  }
}
...
static void test_gyro_damping() {
  ...
  // Assert 1: Force opposes movement (should be negative for positive steering
  // velocity) Steering moved from 0.0 to 0.1 (positive direction) Gyro damping
  // should oppose this (negative force)
  if (gyro_force < 0.0) {
    std::cout << "[PASS] Gyro force opposes steering movement (negative: "
              << gyro_force << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Gyro force should be negative. Got: " << gyro_force
              << std::endl;
    g_tests_failed++;
  }
  ...
  if (std::abs(gyro_force) > 0.001) {
    std::cout << "[PASS] Gyro force is non-zero (magnitude: "
              << std::abs(gyro_force) << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Gyro force is too small. Got: " << gyro_force
              << std::endl;
    g_tests_failed++;
  }
  ...
  if (!batch.empty()) {
    ...
    if (gyro_force_reverse > 0.0) {
      std::cout
          << "[PASS] Gyro force reverses with steering direction (positive: "
          << gyro_force_reverse << ")" << std::endl;
      g_tests_passed++;
    } else {
      std::cout
          << "[FAIL] Gyro force should be positive for reverse movement. Got: "
          << gyro_force_reverse << std::endl;
      g_tests_failed++;
    }
  }
  ...
  if (!batch.empty()) {
    ...
    if (std::abs(gyro_force_slow) < std::abs(gyro_force) * 0.6) {
      std::cout << "[PASS] Gyro force scales with speed (slow: "
                << gyro_force_slow << " vs fast: " << gyro_force << ")"
                << std::endl;
      g_tests_passed++;
    } else {
      std::cout << "[FAIL] Gyro force should be weaker at low speed. Slow: "
                << gyro_force_slow << " Fast: " << gyro_force << std::endl;
      g_tests_failed++;
    }
  }
}
...
// ========================================
// These tests verify the fixes for the rFactor 2 / LMU coordinate system
// mismatch. The game uses a left-handed system (+X = left), while DirectInput
// uses standard (+X = right). Without proper inversions, FFB effects fight the
// physics instead of helping.
...
static void test_coordinate_sop_inversion() {
  ...
  engine.m_sop_smoothing_factor =
      1.0f; // Disable smoothing for instant response
  engine.m_gain = 1.0f;
  engine.m_max_torque_ref = 20.0f;
  engine.m_understeer_effect = 0.0f;
  engine.m_rear_align_effect = 0.0f;
  ...
  // Expected: Wheel should pull LEFT (negative force) to simulate heavy
  // steering
  ...
  if (force > 0.4) {
    std::cout << "[PASS] SoP pulls LEFT in right turn (force: " << force << ")"
              << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] SoP should pull LEFT (Positive). Got: " << force
  ...
  if (force < -0.4) {
    std::cout << "[PASS] SoP pulls RIGHT in left turn (force: " << force << ")"
              << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] SoP should pull RIGHT (Negative). Got: " << force
              << " Expected < -0.4" << std::endl;
    g_tests_failed++;
  }
}
...
static void test_coordinate_rear_torque_inversion() {
  ...
  if (force < -0.2) {
    std::cout << "[PASS] Rear torque provides counter-steer LEFT (force: "
              << force << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Rear torque should counter-steer LEFT. Got: " << force
  ...
  // v0.4.19 FIX: After removing abs() from slip angle, this should now work
  // correctly!
  ...
  // Lat force = -0.245 * 4300 * 15.0 ≈ -15817 N (clamped to -6000 N)
  // Torque = -(-6000) * 0.001 * 1.0 = +6.0 Nm (POSITIVE for right
  // counter-steer) Normalized = +6.0 / 20.0 = +0.3

  if (force > 0.2) {
    std::cout << "[PASS] Rear torque provides counter-steer RIGHT (force: "
              << force << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Rear torque should counter-steer RIGHT. Got: " << force
              << " Expected > 0.2" << std::endl;
    g_tests_failed++;
  }
}
...
static void test_coordinate_scrub_drag_direction() {
  ...
  if (force < -0.2) {
    std::cout << "[PASS] Scrub drag opposes left slide (Torque Left: " << force
              << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Scrub drag direction wrong. Got: " << force
              << " Expected < -0.2" << std::endl;
    g_tests_failed++;
  }
  ...
  if (force > 0.2) {
    std::cout << "[PASS] Scrub drag opposes right slide (Torque Right: "
              << force << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Scrub drag direction wrong. Got: " << force
...
static void test_coordinate_debug_slip_angle_sign() {
  ...
  // This test verifies that calculate_raw_slip_angle_pair() preserves sign
  // information for debug visualization (snap.raw_front_slip_angle and
  // snap.raw_rear_slip_angle)
  ...
  if (snap.raw_front_slip_angle > 0.2) {
    std::cout << "[PASS] Front slip angle is POSITIVE for left slide ("
              << snap.raw_front_slip_angle << " rad)" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Front slip angle should be POSITIVE. Got: "
              << snap.raw_front_slip_angle << std::endl;
    g_tests_failed++;
  }
  ...
  if (!batch.empty()) {
    ...
    if (snap.raw_front_slip_angle < -0.2) {
      std::cout << "[PASS] Front slip angle is NEGATIVE for right slide ("
                << snap.raw_front_slip_angle << " rad)" << std::endl;
      g_tests_passed++;
    } else {
      std::cout << "[FAIL] Front slip angle should be NEGATIVE. Got: "
                << snap.raw_front_slip_angle << std::endl;
      g_tests_failed++;
    }
  }
  ...
  if (!batch.empty()) {
    ...
    if (snap.raw_rear_slip_angle > 0.2) {
      std::cout << "[PASS] Rear slip angle is POSITIVE for left slide ("
                << snap.raw_rear_slip_angle << " rad)" << std::endl;
      g_tests_passed++;
    } else {
      std::cout << "[FAIL] Rear slip angle should be POSITIVE. Got: "
                << snap.raw_rear_slip_angle << std::endl;
      g_tests_failed++;
    }
  }
  ...
  if (!batch.empty()) {
    ...
    // Expected: atan2(-5.0, 20.0) ≈ -0.245 rad (NEGATIVE)
    if (snap.raw_rear_slip_angle < -0.2) {
      std::cout << "[PASS] Rear slip angle is NEGATIVE for right slide ("
                << snap.raw_rear_slip_angle << " rad)" << std::endl;
      g_tests_passed++;
    } else {
      std::cout << "[FAIL] Rear slip angle should be NEGATIVE. Got: "
                << snap.raw_rear_slip_angle << std::endl;
      g_tests_failed++;
    }
  }
...
static void test_regression_no_positive_feedback() {
  ...
  // This was caused by inverted rear aligning torque creating positive
  // feedback.
  ...
  data.mWheel[2].mLateralPatchVel =
      -5.0; // Sliding left (ISO Coords for Rear Torque)
  data.mWheel[3].mLateralPatchVel = -5.0;
  data.mWheel[2].mLongitudinalGroundVel = 20.0;
  data.mWheel[3].mLongitudinalGroundVel = 20.0;
  data.mWheel[2].mSuspForce = 4000.0;
  ...
  // 3. Scrub Drag pushes LEFT (Positive) - with -Vel input (Destabilizing but
  // consistent with code)
  //
  // The combination should result in a net STABILIZING force (SoP Dominates).

  if (force > 0.0) {
    std::cout << "[PASS] Combined forces are stabilizing (net left pull: "
              << force << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Combined forces should pull LEFT (Positive). Got: "
              << force << std::endl;
    g_tests_failed++;
  }
  ...
  if (!batch.empty()) {
    ...
    if (snap.sop_force > 0.0) {
      std::cout << "[PASS] SoP component is Positive (" << snap.sop_force << ")"
                << std::endl;
      g_tests_passed++;
    } else {
      std::cout << "[FAIL] SoP should be Positive. Got: " << snap.sop_force
                << std::endl;
      g_tests_failed++;
    }
    ...
    if (snap.ffb_rear_torque > 0.0) {
      std::cout << "[PASS] Rear torque is Positive (" << snap.ffb_rear_torque
                << ")" << std::endl;
      g_tests_passed++;
    } else {
      std::cout << "[FAIL] Rear torque should be Positive. Got: "
                << snap.ffb_rear_torque << std::endl;
      g_tests_failed++;
    }

    // Scrub drag Positive (with -Vel input)
    if (snap.ffb_scrub_drag > 0.0) {
      std::cout << "[PASS] Scrub drag is Positive (" << snap.ffb_scrub_drag
                << ")" << std::endl;
      g_tests_passed++;
    } else {
      std::cout << "[FAIL] Scrub drag should be Positive. Got: "
                << snap.ffb_scrub_drag << std::endl;
      g_tests_failed++;
    }
  }
}
...
static void test_coordinate_all_effects_alignment() {
  ...
  data.mWheel[2].mLateralPatchVel =
      -5.0; // Rear Sliding Left (Negative Vel for Correct Code Physics)
  data.mWheel[3].mLateralPatchVel = -5.0;
  ...
  if (snap.sop_force < 0.1) {
    std::cout << "[FAIL] SoP fighting alignment! Val: " << snap.sop_force
              << std::endl;
    all_aligned = false;
  }
  ...
  if (snap.ffb_rear_torque < 0.1) {
    std::cout << "[FAIL] Rear Torque fighting alignment! Val: "
              << snap.ffb_rear_torque << std::endl;
    all_aligned = false;
  }
  ...
  if (snap.ffb_yaw_kick > -0.1) {
    std::cout << "[FAIL] Yaw Kick fighting alignment! Val: "
              << snap.ffb_yaw_kick << std::endl;
    all_aligned = false;
  }

  // 4. Scrub Drag (Should be Negative)
  if (snap.ffb_scrub_drag > -0.01) {
    std::cout << "[FAIL] Scrub Drag fighting alignment! Val: "
              << snap.ffb_scrub_drag << std::endl;
    all_aligned = false;
  }
...
static void test_regression_phase_explosion() {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    if (engine.m_slide_phase < -0.001 || engine.m_slide_phase > 6.30) {
      std::cout << "[FAIL] Slide Phase out of bounds: " << engine.m_slide_phase
                << std::endl;
      failed = true;
    }
    if (engine.m_lockup_phase < -0.001 || engine.m_lockup_phase > 6.30) {
      std::cout << "[FAIL] Lockup Phase out of bounds: "
                << engine.m_lockup_phase << std::endl;
      failed = true;
    }
    if (engine.m_spin_phase < -0.001 || engine.m_spin_phase > 6.30) {
      std::cout << "[FAIL] Spin Phase out of bounds: " << engine.m_spin_phase
                << std::endl;
      failed = true;
    }
  }
...
static void test_time_corrected_smoothing() {
  ...
  data.mDeltaTime = 0.0025;
  for (int i = 0; i < 80; i++)
    engine_fast.calculate_force(&data);

  // Slow: dt = 0.02, 10 steps = 0.2s
  data.mDeltaTime = 0.02;
  for (int i = 0; i < 10; i++)
    engine_slow.calculate_force(&data);
  ...
  std::cout << "Fast Yaw (400Hz): " << val_fast
            << " Slow Yaw (50Hz): " << val_slow << std::endl;

  // Tolerance: 5% (Integration difference is expected)
  if (std::abs(val_fast - val_slow) < 0.5) {
    std::cout << "[PASS] Smoothing is consistent across frame rates."
              << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Smoothing diverges! Time correction failed."
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_gyro_stability() {
  ...
  engine.m_gyro_smoothing =
      -1.0; // Malicious input (should be clamped to 0.0 internally)
  ...
  // Check if exploded
  if (std::abs(engine.m_steering_velocity_smoothed) < 1000.0 &&
      !std::isnan(engine.m_steering_velocity_smoothed)) {
...
void test_kinematic_load_braking() {
  ...
  if (load > 4000.0) {
    std::cout << "[PASS] Front Load Increased under Braking (Approx " << load
              << " N)" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Front Load did not increase significantly. Value: "
              << load << std::endl;
    g_tests_failed++;
  }
}
...
void test_combined_grip_loss() {
  ...
  if (grip < 0.5) {
    std::cout << "[PASS] Grip dropped due to Longitudinal Slip (" << grip << ")"
              << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Grip remained high despite lockup. Value: " << grip
              << std::endl;
    g_tests_failed++;
  }
}
...
void test_chassis_inertia_smoothing_convergence() {
  ...
  if (smoothed_x > expected && smoothed_z > expected) {
    std::cout << "[PASS] Smoothing converged (X: " << smoothed_x
              << ", Z: " << smoothed_z << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Smoothing did not converge. X: " << smoothed_x
              << " Z: " << smoothed_z << " Expected > " << expected
              << std::endl;
    g_tests_failed++;
  }
  ...
  if (smoothed_x < 0.5 && smoothed_z < 0.5) {
    std::cout << "[PASS] Smoothing decayed correctly (X: " << smoothed_x
              << ", Z: " << smoothed_z << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Smoothing did not decay. X: " << smoothed_x
              << " Z: " << smoothed_z << std::endl;
    g_tests_failed++;
  }
}
...
void test_kinematic_load_cornering() {
  ...
  if (load_fl > load_fr) {
    std::cout << "[PASS] Left wheel has more load in right turn (FL: "
              << load_fl << "N, FR: " << load_fr << "N)" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Lateral transfer incorrect. FL: " << load_fl
              << " FR: " << load_fr << std::endl;
    g_tests_failed++;
  }
  ...
  if (diff > 2000.0 && diff < 2800.0) {
    std::cout << "[PASS] Lateral transfer magnitude reasonable (" << diff
              << "N)" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Lateral transfer magnitude unexpected: " << diff
              << "N (expected ~2400N)" << std::endl;
    g_tests_failed++;
  }
  ...
  if (load_fr > load_fl) {
    std::cout << "[PASS] Right wheel has more load in left turn (FR: "
              << load_fr << "N, FL: " << load_fl << "N)" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Lateral transfer reversed incorrectly. FL: " << load_fl
              << " FR: " << load_fr << std::endl;
    g_tests_failed++;
  }
}
...
static void test_static_notch_integration() {
  ...
  engine.m_static_notch_width =
      10.0; // Q = 11/10 = 1.1 (Wide notch for testing)
  engine.m_gain = 1.0;
  ...
  for (int i = 0; i < 400; i++) { // 1 second
    double t = (double)i * data.mDeltaTime;
    data.mSteeringShaftTorque =
        std::sin(2.0 * 3.14159265 * 11.0 * t); // Test at 11Hz
  ...
  if (max_amp_target < 0.3) {
    std::cout << "[PASS] Static Notch attenuated 11Hz signal (Max Amp: "
              << max_amp_target << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Static Notch failed to attenuate 11Hz. Max Amp: "
              << max_amp_target << std::endl;
    g_tests_failed++;
  }
  ...
  for (int i = 0; i < 400; i++) {
    double t = (double)i * data.mDeltaTime;
    data.mSteeringShaftTorque =
        std::sin(2.0 * 3.14159265 * 20.0 * t); // Test at 20Hz (far from 11Hz)
  ...
  if (max_amp_pass > 0.8) {
    std::cout << "[PASS] Static Notch passed 20Hz signal (Max Amp: "
              << max_amp_pass << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Static Notch attenuated 20Hz signal. Max Amp: "
              << max_amp_pass << std::endl;
    g_tests_failed++;
  }
}
...
static void test_gain_compensation() {
  ...
  double ra1, ra2;
  {
    FFBEngine e1;
    e1.m_gain = 1.0;
    e1.m_invert_force = false;
    e1.m_understeer_effect = 0.0;
    e1.m_oversteer_boost = 0.0;
  ...
  }
  {
    FFBEngine e2;
    e2.m_gain = 1.0;
    e2.m_invert_force = false;
    e2.m_understeer_effect = 0.0;
    e2.m_oversteer_boost = 0.0;
  ...
  if (std::abs(ra1 - ra2) < 0.001) {
    std::cout << "[PASS] Rear Align Torque correctly compensated (" << ra1
              << " == " << ra2 << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Rear Align Torque compensation failed! 20Nm: " << ra1
              << " 60Nm: " << ra2 << std::endl;
    g_tests_failed++;
  }
  ...
  if (std::abs(s1 - s2) < 0.001) {
    std::cout << "[PASS] Slide Texture correctly compensated (" << s1
              << " == " << s2 << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Slide Texture compensation failed! 20Nm: " << s1
              << " 100Nm: " << s2 << std::endl;
    g_tests_failed++;
  }
  ...
  if (std::abs(u1 - (u2 * 2.0)) < 0.001) {
    std::cout << "[PASS] Understeer Modifier correctly uncompensated (" << u1
              << " vs " << u2 << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Understeer Modifier behavior unexpected! 20Nm: " << u1
              << " 40Nm: " << u2 << std::endl;
    g_tests_failed++;
  }

  std::cout << "[SUMMARY] Gain Compensation verified for all effect types."
            << std::endl;
}
...
static void test_config_safety_clamping() {
  ...
  if (engine.m_slide_texture_gain != 2.0f) {
    std::cout << "[FAIL] slide_gain not clamped. Got: "
              << engine.m_slide_texture_gain << " Expected: 2.0" << std::endl;
    all_clamped = false;
  }
  if (engine.m_road_texture_gain != 2.0f) {
    std::cout << "[FAIL] road_gain not clamped. Got: "
              << engine.m_road_texture_gain << " Expected: 2.0" << std::endl;
    all_clamped = false;
  }
  if (engine.m_lockup_gain != 3.0f) {
    std::cout << "[FAIL] lockup_gain not clamped. Got: " << engine.m_lockup_gain
              << " Expected: 3.0" << std::endl;
    all_clamped = false;
  }
  if (engine.m_spin_gain != 2.0f) {
    std::cout << "[FAIL] spin_gain not clamped. Got: " << engine.m_spin_gain
              << " Expected: 2.0" << std::endl;
    all_clamped = false;
  }
  if (engine.m_rear_align_effect != 2.0f) {
    std::cout << "[FAIL] rear_align_effect not clamped. Got: "
              << engine.m_rear_align_effect << " Expected: 2.0" << std::endl;
    all_clamped = false;
  }
  if (engine.m_sop_yaw_gain != 1.0f) {
    std::cout << "[FAIL] sop_yaw_gain not clamped. Got: "
              << engine.m_sop_yaw_gain << " Expected: 1.0" << std::endl;
    all_clamped = false;
  }
  if (engine.m_sop_effect != 2.0f) {
    std::cout << "[FAIL] sop not clamped. Got: " << engine.m_sop_effect
              << " Expected: 2.0" << std::endl;
    all_clamped = false;
  }
  ...
  if (engine.m_scrub_drag_gain != 1.0f) {
    std::cout << "[FAIL] scrub_drag_gain not clamped. Got: "
              << engine.m_scrub_drag_gain << " Expected: 1.0" << std::endl;
    all_clamped = false;
  }
  if (engine.m_gyro_gain != 1.0f) {
    std::cout << "[FAIL] gyro_gain not clamped. Got: " << engine.m_gyro_gain
              << " Expected: 1.0" << std::endl;
    all_clamped = false;
  }

  if (all_clamped) {
    std::cout << "[PASS] All legacy high-gain values correctly clamped to safe "
                 "maximums."
              << std::endl;
    g_tests_passed++;
  } else {
...
static void test_grip_threshold_sensitivity() {
  ...
  // Verify: post-peak sensitive car should have LESS grip than GT3 car at same
  // slip
  if (grip_sensitive_post < grip_gt3) {
    std::cout << "[PASS] Sensitive car (0.06) lost more grip at 0.07 slip than "
                 "GT3 car (0.12)."
              << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Sensitivity threshold not working. S: "
              << grip_sensitive_post << " G: " << grip_gt3 << std::endl;
    g_tests_failed++;
  }
}
...
static void test_steering_shaft_smoothing() {
  ...
  if (std::abs(force - 0.166) < 0.01) {
    std::cout << "[PASS] Shaft Smoothing delayed the step input (Frame 1: "
              << force << ")." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Shaft Smoothing mismatch. Got " << force
              << " Expected ~0.166." << std::endl;
    g_tests_failed++;
  }
  ...
  if (force > 0.8 && force < 0.95) {
    std::cout << "[PASS] Shaft Smoothing converged correctly (Frame 11: "
              << force << ")." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Shaft Smoothing convergence failure. Got " << force
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_config_defaults_v057() {
  ...
  if (Config::m_always_on_top == true) {
    ...
    std::cout << "[FAIL] 'Always on Top' is DISABLED by default (Regression)."
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_config_safety_validation_v057() {
  ...
  // Create a temporary config file with invalid values that would cause
  // division-by-zero
  const char *test_file = "tmp_invalid_grip_config_test.ini";
  {
    ...
    // Write dangerous values that would cause division-by-zero in grip
    // calculations
  ...
  if (engine.m_optimal_slip_angle == 0.10f) {
    std::cout << "[PASS] Invalid optimal_slip_angle (0.0) reset to safe "
                 "default (0.10)."
              << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] optimal_slip_angle not reset. Got: "
              << engine.m_optimal_slip_angle << " Expected: 0.10" << std::endl;
    g_tests_failed++;
    all_safe = false;
  }
  ...
  if (engine.m_optimal_slip_ratio == 0.12f) {
    std::cout << "[PASS] Invalid optimal_slip_ratio (0.0) reset to safe "
                 "default (0.12)."
              << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] optimal_slip_ratio not reset. Got: "
              << engine.m_optimal_slip_ratio << " Expected: 0.12" << std::endl;
    g_tests_failed++;
    all_safe = false;
  }
  ...
  if (engine.m_gain == 1.5f) {
    std::cout << "[PASS] Valid config values still loaded correctly (gain=1.5)."
              << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Valid values not loaded. Got gain: " << engine.m_gain
  ...
  if (engine2.m_optimal_slip_angle == 0.10f &&
      engine2.m_optimal_slip_ratio == 0.12f) {
    std::cout << "[PASS] Very small values (<0.01) correctly reset to defaults."
              << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Small value validation failed. Angle: "
              << engine2.m_optimal_slip_angle
  ...
  if (all_safe) {
    std::cout << "[SUMMARY] All division-by-zero protections working correctly."
              << std::endl;
  }
}
...
static void test_rear_lockup_differentiation() {
  ...
  if (phase_delta_front > 0.0) {
    std::cout << "[PASS] Front lockup triggered. Phase delta: "
              << phase_delta_front << std::endl;
    g_tests_passed++;
  } else {
  ...
  if (phase_delta_rear > 0.0) {
    std::cout << "[PASS] Rear lockup triggered. Phase delta: "
              << phase_delta_rear << std::endl;
    g_tests_passed++;
  } else {
  ...
  if (std::abs(ratio - 0.3) < 0.05) {
    std::cout << "[PASS] Rear frequency is lower (Ratio: " << ratio
              << " vs expected 0.3)." << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Frequency differentiation failed. Ratio: " << ratio
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_split_load_caps() {
  ...
  for (int i = 0; i < 4; i++)
    data.mWheel[i].mTireLoad = 12000.0;
  ...
  if (std::abs(force_road - 0.05) < 0.001) {
    std::cout << "[PASS] Road texture correctly clamped to 1.0x (Force: "
              << force_road << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Road texture clamping failed. Expected 0.05, got "
              << force_road << std::endl;
    g_tests_failed++;
  ...
  data.mWheel[1].mLongitudinalPatchVel =
      -10.0; // Slip (both wheels for consistency)
  ...
  // Expected: force_high ≈ 3.0 * force_low (within tolerance for phase
  // differences)
  double expected_ratio = 3.0;
  double actual_ratio =
      std::abs(force_high) /
      (std::abs(force_low) + 0.0001); // Add epsilon to avoid div-by-zero

  // Use a tolerance of ±0.5 to account for phase integration differences
  if (std::abs(actual_ratio - expected_ratio) < 0.5) {
    std::cout << "[PASS] Brake load cap applies 3x scaling (Ratio: "
              << actual_ratio << ", High: " << std::abs(force_high)
              << ", Low: " << std::abs(force_low) << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Expected ~3x ratio, got " << actual_ratio
              << " (High: " << std::abs(force_high)
              << ", Low: " << std::abs(force_low) << ")" << std::endl;
    g_tests_failed++;
  }
}
...
static void test_dynamic_thresholds() {
  ...
  // Both should have non-zero force, and max should be significantly higher due
  // to quadratic ramp 10% slip: severity = (0.5)^2 = 0.25 20% slip: severity
  // = 1.0
...
static void test_predictive_lockup_v060() {
  ...
  engine.m_lockup_start_pct = 5.0f;
  engine.m_lockup_full_pct =
      15.0f; // Default threshold is higher than current slip

  data.mUnfilteredBrake =
      1.0; // Needs brake input for prediction gating (v0.6.0)
  ...
  // With 10% slip and prediction active, threshold is 5%, so severity is
  // (10-5)/10 = 0.5. Phase should advance.

  if (engine.m_lockup_phase > 0.001) {
    std::cout << "[PASS] Predictive trigger activated at 10% slip (Phase: "
              << engine.m_lockup_phase << ")" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Predictive trigger failed. Phase: "
              << engine.m_lockup_phase
              << " Accel: " << (data.mWheel[0].mRotation - prev_rot) / 0.01
              << std::endl;
    g_tests_failed++;
  }
}
...
static void test_abs_pulse_v060() {
  ...
  InitializeEngine(engine);
  TelemInfoV01 data =
      CreateBasicTestTelemetry(20.0); // Moving car (v0.6.21 FIX)
  ...
  if (std::abs(force) > 0.001) {
    std::cout << "[PASS] ABS Pulse triggered (Force: " << force << ")"
              << std::endl;
    g_tests_passed++;
  } else {
...
static void test_missing_telemetry_warnings() {
  ...
  bool grip_warn =
      output.find(
          "Warning: Data for mGripFract from the game seems to be missing for "
          "this car (TestCar_GT3). (Likely Encrypted/DLC Content)") !=
      std::string::npos;
  ...
  if (grip_warn) {
    ...
    std::cout << "[FAIL] Grip warning missing or format incorrect."
              << std::endl;
    g_tests_failed++;
  ...
  bool susp_warn =
      output.find(
          "Warning: Data for mSuspForce from the game seems to be missing for "
          "this car (TestCar_GT3). (Likely Encrypted/DLC Content)") !=
      std::string::npos;

  if (susp_warn) {
    std::cout.rdbuf(prev_cout_buf);
    std::cout << "[PASS] SuspForce warning triggered with car name."
              << std::endl;
    g_tests_passed++;
    std::cout.rdbuf(buffer.rdbuf());
  } else {
    std::cout.rdbuf(prev_cout_buf);
    std::cout << "[FAIL] SuspForce warning missing or format incorrect."
              << std::endl;
    g_tests_failed++;
    std::cout.rdbuf(buffer.rdbuf());
  }
  ...
  for (int i = 0; i < 4; i++)
    data.mWheel[i].mVerticalTireDeflection = 0.0;
  ...
  output = buffer.str();
  bool vert_warn =
      output.find("[WARNING] mVerticalTireDeflection is missing") !=
      std::string::npos;
...
static void test_notch_filter_bandwidth() {
  ...
  for (int i = 0; i < 100; i++) {
    data.mSteeringShaftTorque =
        std::sin(2.0 * PI * 50.0 * (i * data.mDeltaTime)) * amplitude;
    double output = std::abs(engine.calculate_force(&data));
    if (i > 50 && output > max_output)
      max_output = output;
  }
  ...
  for (int i = 0; i < 100; i++) {
    data.mSteeringShaftTorque =
        std::sin(2.0 * PI * 46.0 * (i * data.mDeltaTime)) * amplitude;
    double output = std::abs(engine.calculate_force(&data));
    if (i > 50 && output > max_output)
      max_output = output;
  }
  // 46Hz is within the 10Hz bandwidth (45-55). Should be significantly
  // attenuated but > 0. Max unattenuated is 0.5. Calculated gain ~0.64 ->
  // Expect ~0.32
  ...
  for (int i = 0; i < 100; i++) {
    data.mSteeringShaftTorque =
        std::sin(2.0 * PI * 65.0 * (i * data.mDeltaTime)) * amplitude;
    double output = std::abs(engine.calculate_force(&data));
    if (i > 50 && output > max_output)
      max_output = output;
  }
...
static void test_notch_filter_edge_cases() {
  ...
  for (int i = 0; i < 100; i++) {
    data.mSteeringShaftTorque =
        std::sin(2.0 * PI * 11.0 * (i * data.mDeltaTime)) * amplitude;
    double output = std::abs(engine.calculate_force(&data));
    if (i > 50 && output > max_output_narrow)
      max_output_narrow = output;
  }
  // Notch filter with high Q provides excellent attenuation but not perfect due
  // to transients
  ASSERT_TRUE(max_output_narrow <
              0.6); // Very narrow notch still attenuates center significantly

  // Test at 10.5Hz (just 0.5 Hz away) - should pass through with narrow notch
  max_output_narrow = 0.0;
  for (int i = 0; i < 100; i++) {
    data.mSteeringShaftTorque =
        std::sin(2.0 * PI * 10.5 * (i * data.mDeltaTime)) * amplitude;
    double output = std::abs(engine.calculate_force(&data));
    if (i > 50 && output > max_output_narrow)
      max_output_narrow = output;
  }
  ASSERT_TRUE(max_output_narrow >
              0.3); // Narrow notch doesn't affect nearby frequencies
  ...
  for (int i = 0; i < 100; i++) {
    data.mSteeringShaftTorque =
        std::sin(2.0 * PI * 6.0 * (i * data.mDeltaTime)) * amplitude;
    double output = std::abs(engine.calculate_force(&data));
    if (i > 50 && output > max_output_wide)
      max_output_wide = output;
  }
  ...
  ASSERT_TRUE(max_output_narrow <
              0.7); // Safety clamp prevents extreme Q values
}
...
static void test_yaw_kick_edge_cases() {
  ...
  ASSERT_TRUE(std::abs(force_tiny) >
              0.001); // With zero threshold, signals pass
...
static void test_stationary_silence() {
  ...
  if (std::abs(force) > 0.001) {
    std::cout << "  [DEBUG] Stationary Silence Fail: force=" << force
              << std::endl;
...
static void test_stationary_gate() {
  ...
  // Case 2: Moving slowly (0.5 m/s) -> Gate should be 0.0 (since 0.5 <
  // m_speed_gate_lower)
  {
...
static void test_idle_smoothing() {
  ...
  if (max_force_idle < 0.15) {
    std::cout << "[PASS] Idle vibration attenuated (Max: " << max_force_idle
              << " < 0.15)" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Idle vibration too strong! Max: " << max_force_idle
              << std::endl;
    g_tests_failed++;
  }
  ...
  for (int i = 0; i < 100; i++) {
    ...
    data_driving.mSteeringShaftTorque =
        5.0 * std::sin(20.0 * 6.28 * t); // Same vibration (e.g. curb)
  ...
  if (max_force_driving > 0.4) {
    std::cout << "[PASS] Driving vibration passed through (Max: "
              << max_force_driving << " > 0.4)" << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Driving vibration over-smoothed. Max: "
              << max_force_driving << std::endl;
    g_tests_failed++;
  }
}
...
static void test_speed_gate_custom_thresholds() {
  ...
  if (engine.m_speed_gate_upper == 5.0f) {
    std::cout << "[PASS] Default upper threshold is 5.0 m/s (18 km/h)."
              << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] Default upper threshold is "
              << engine.m_speed_gate_upper << std::endl;
    g_tests_failed++;
  }
...
static void test_grip_floor_clamp() {
  ...
  // GRIP_FLOOR_CLAMP: The grip estimator in FFBEngine.h (line 622) enforces a
  // minimum grip value of 0.2 to prevent total force loss even under extreme
  // slip conditions. This safety floor ensures the wheel never goes completely
  // dead.
...
static void test_understeer_effect_scaling() {
  ...
  double f0 = 0.0;
  for (int i = 0; i < FILTER_SETTLING_FRAMES; i++)
    f0 = engine.calculate_force(&data);

  engine.m_understeer_effect = 1.0f;
  double f1 = 0.0;
  for (int i = 0; i < FILTER_SETTLING_FRAMES; i++)
    f1 = engine.calculate_force(&data);

  engine.m_understeer_effect = 2.0f;
  double f2 = 0.0;
  for (int i = 0; i < FILTER_SETTLING_FRAMES; i++)
    f2 = engine.calculate_force(&data);
...
static void test_preset_understeer_only_isolation() {
  ...
  ASSERT_NEAR(p.optimal_slip_angle, 0.10f,
              0.001f); // Optimal slip angle threshold
  ASSERT_NEAR(p.optimal_slip_ratio, 0.12f,
              0.001f);                 // Optimal slip ratio threshold
  ...
  std::cout << "[PASS] 'Test: Understeer Only' preset properly isolates "
               "understeer effect"
            << std::endl;
  g_tests_passed++;
}
...
static void test_all_presets_non_negative_speed_gate() {
  ...
  for (size_t i = 0; i < Config::presets.size(); i++) {
    ...
    if (p.speed_gate_lower < 0.0f) {
      std::cout << "[FAIL] Preset '" << p.name
                << "' has negative speed_gate_lower: " << p.speed_gate_lower
                << " m/s (" << (p.speed_gate_lower * 3.6f) << " km/h)"
                << std::endl;
      all_valid = false;
    }
    ...
    if (p.speed_gate_upper < 0.0f) {
      std::cout << "[FAIL] Preset '" << p.name
                << "' has negative speed_gate_upper: " << p.speed_gate_upper
                << " m/s (" << (p.speed_gate_upper * 3.6f) << " km/h)"
                << std::endl;
      all_valid = false;
    }
    ...
    if (p.speed_gate_upper < p.speed_gate_lower) {
      std::cout << "[FAIL] Preset '" << p.name
                << "' has speed_gate_upper < speed_gate_lower: "
  ...
  if (all_valid) {
    std::cout << "[PASS] All " << Config::presets.size()
              << " presets have valid non-negative speed gate values"
              << std::endl;
    g_tests_passed++;
  } else {
    std::cout << "[FAIL] One or more presets have invalid speed gate values"
              << std::endl;
    g_tests_failed++;
  }
}
