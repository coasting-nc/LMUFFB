...
double FFBEngine::calculate_force(const TelemInfoV01* data, const char* vehicleClass, const char* vehicleName, float genFFBTorque) {
    ...
    // v0.7.72: Added toggle for Dynamic Normalization on legacy source. If disabled, use wheelbase max as reference.
    double target_structural_mult = (m_torque_source == 1 || !m_dynamic_normalization_enabled) ? (1.0 / (m_wheelbase_max_nm + 1e-9)) : (1.0 / (m_session_peak_torque + 1e-9));
