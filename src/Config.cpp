    ...
    if (std::getline(is_line, key, '=')) {
        ...
        if (std::getline(is_line, value)) {
            try {
                ...
                else if (key == "slope_confidence_max_rate") current_preset.slope_confidence_max_rate = std::stof(value); // NEW v0.7.40
            } catch (...) {}
        }
    }
}
...
void Config::Load(FFBEngine& engine, const std::string& filename) {
        ...
        if (std::getline(is_line, key, '=')) {
            ...
            if (std::getline(is_line, value)) {
                try {
                    ...
                    else if (key == "slope_confidence_max_rate") engine.m_slope_confidence_max_rate = std::stof(value); // NEW v0.7.40
                } catch (...) {
    ...
    engine.m_slope_confidence_max_rate = (std::max)(0.01f, (std::min)(2.0f, engine.m_slope_confidence_max_rate));
