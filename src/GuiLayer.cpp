...
// Suppress deprecation warning for sprintf in stb_image_write.h (third-party
// library)
#pragma warning(push)
...
// Global DirectX variables (Simplified for brevity, usually managed in a
...
static bool show_unsaved_preset_warning = false;

// v0.5.5 Layout Constants
static const float CONFIG_PANEL_WIDTH =
    500.0f; // Width of config panel when graphs are visible
static const int MIN_WINDOW_WIDTH =
    400; // Minimum window width to keep UI usable
static const int MIN_WINDOW_HEIGHT =
    600; // Minimum window height to keep UI usable
 ...
 *
 * @param is_graph_mode If true, saves to win_w_large/win_h_large; otherwise to
 * win_w_small/win_h_small
 */
void SaveCurrentWindowGeometry(bool is_graph_mode) {
  RECT rect;
  if (::GetWindowRect(g_hwnd, &rect)) {
    Config::Get().SetWinPosX(rect.left);
    Config::Get().SetWinPosY(rect.top);
    ...
    if (is_graph_mode) {
      Config::Get().SetWinWidthLarge(w);
      Config::Get().SetWinHeightLarge(h);
    } else {
      Config::Get().SetWinWidthSmall(w);
      Config::Get().SetWinHeightSmall(h);
    }
  }
}
...
bool GuiLayer::Init() {
  ...
  WNDCLASSEXW wc = {
      sizeof(wc), CS_NOCLOSE, WndProc, 0L,   0L,        GetModuleHandle(NULL),
      NULL,       NULL,       NULL,    NULL, L"lmuFFB", NULL};
  ::RegisterClassExW(&wc);
  ...
  int start_w = Config::Get().GetShowGraphs()
                    ? Config::Get().GetWinWidthLarge()
                    : Config::Get().GetWinWidthSmall();
  int start_h = Config::Get().GetShowGraphs()
                    ? Config::Get().GetWinHeightLarge()
                    : Config::Get().GetWinHeightSmall();

  // Enforce minimum dimensions
  if (start_w < MIN_WINDOW_WIDTH)
    start_w = MIN_WINDOW_WIDTH;
  if (start_h < MIN_WINDOW_HEIGHT)
    start_h = MIN_WINDOW_HEIGHT;

  // 2. Validate window position (ensure it's on-screen)
  int pos_x = Config::Get().GetWinPosX();
  int pos_y = Config::Get().GetWinPosY();

  // Get primary monitor work area
  RECT workArea;
  SystemParametersInfo(SPI_GETWORKAREA, 0, &workArea, 0);

  // If saved position would place window completely off-screen, reset to
  // default
...
void GuiLayer::Shutdown(FFBEngine &engine) {
  ...
  SaveCurrentWindowGeometry(Config::Get().GetShowGraphs());
...
void *GuiLayer::GetWindowHandle() { return (void *)g_hwnd; }
...
bool GuiLayer::Render(FFBEngine &engine) {
  ...
  // Note: In a real app we'd check IsIconic(g_hwnd) outside this logic or
  // return a 'should_sleep' flag
  if (g_running == false) {
    return false;
  }
...
// Helper function to capture a window using PrintWindow API (works for console
// and all window types)
bool CaptureWindowToBuffer(HWND hwnd, std::vector<unsigned char> &buffer,
                           int &width, int &height) {
  if (!hwnd || !IsWindow(hwnd)) {
    std::cout << "[DEBUG] CaptureWindowToBuffer failed: Invalid window handle"
              << std::endl;
    return false;
  }
  ...
  if (!GetWindowRect(hwnd, &rect)) {
    std::cout
        << "[DEBUG] CaptureWindowToBuffer failed: GetWindowRect failed, error: "
        << GetLastError() << std::endl;
    return false;
  }

  std::cout << "[DEBUG] GetWindowRect returned: left=" << rect.left
  ...
  if (width <= 0 || height <= 0) {
    std::cout << "[DEBUG] GetWindowRect returned invalid dimensions, trying "
                 "GetClientRect..."
              << std::endl;
    ...
    if (GetClientRect(hwnd, &clientRect)) {
      ...
      std::cout << "[DEBUG] GetClientRect returned: " << width << "x" << height
                << std::endl;

      // If we got valid dimensions from GetClientRect, we need to convert to
      // screen coordinates
      if (width > 0 && height > 0) {
        POINT topLeft = {0, 0};
        if (ClientToScreen(hwnd, &topLeft)) {
          ...
          std::cout << "[DEBUG] Converted to screen coordinates: (" << rect.left
                    << "," << rect.top << ") to (" << rect.right << ","
  ...
  std::cout << "[DEBUG] Final calculated dimensions: " << width << "x" << height
            << std::endl;

  if (width <= 0 || height <= 0) {
    std::cout << "[DEBUG] CaptureWindowToBuffer failed: Invalid dimensions "
              << width << "x" << height << std::endl;
    std::cout << "[DEBUG] This usually means the window is minimized, hidden, "
  ...
  if (!hdcScreen) {
    std::cout << "[DEBUG] CaptureWindowToBuffer failed: GetDC(NULL) failed"
              << std::endl;
    return false;
  }
  ...
  if (!hdcMemDC) {
    std::cout
        << "[DEBUG] CaptureWindowToBuffer failed: CreateCompatibleDC failed"
        << std::endl;
  ...
  if (!hbmScreen) {
    std::cout
        << "[DEBUG] CaptureWindowToBuffer failed: CreateCompatibleBitmap failed"
        << std::endl;
    DeleteDC(hdcMemDC);
    ReleaseDC(NULL, hdcScreen);
    return false;
  ...
  if (PrintWindow(hwnd, hdcMemDC, PW_RENDERFULLCONTENT)) {
    ...
    std::cout << "[DEBUG] PrintWindow failed, trying BitBlt fallback..."
              << std::endl;
    if (BitBlt(hdcMemDC, 0, 0, width, height, hdcScreen, rect.left, rect.top,
               SRCCOPY)) {
      captureSuccess = true;
      captureMethod = "BitBlt from screen coordinates";
    } else {
      std::cout << "[DEBUG] BitBlt also failed! Error code: " << GetLastError()
                << std::endl;
    }
  }
  ...
  if (!captureSuccess) {
    std::cout
        << "[DEBUG] CaptureWindowToBuffer failed: All capture methods failed"
        << std::endl;
  ...
  std::cout << "[DEBUG] Capture successful using: " << captureMethod
            << std::endl;
  ...
  if (!GetDIBits(hdcMemDC, hbmScreen, 0, height, buffer.data(),
                 (BITMAPINFO *)&bi, DIB_RGB_COLORS)) {
    std::cout << "[DEBUG] CaptureWindowToBuffer failed: GetDIBits failed"
              << std::endl;
    SelectObject(hdcMemDC, hbmOld);
    DeleteObject(hbmScreen);
    DeleteDC(hdcMemDC);
...
void SaveCompositeScreenshot(const char *filename) {
  ...
  // Capture GUI window
  bool hasGui =
      CaptureWindowToBuffer(guiWindow, guiBuffer, guiWidth, guiHeight);
  std::cout << "[GUI] GUI window capture: " << (hasGui ? "SUCCESS" : "FAILED")
            << std::endl;
  ...
  if (consoleWindow) {
    ...
    std::cout << "[GUI] Console window found (HWND: " << consoleWindow
              << "), visible: " << (isVisible ? "YES" : "NO") << std::endl;

    if (isVisible) {
      std::cout << "[GUI] Attempting to capture visible console window..."
                << std::endl;
      ...
      if (GetConsoleScreenBufferInfo(hConsole, &csbi)) {
        ...
        std::cout << "[DEBUG] Console buffer info: " << consoleWidthChars
                  << " cols x " << consoleHeightChars << " rows" << std::endl;
        ...
        int estimatedWidth =
            consoleWidthChars * fontWidth + 20; // 20px for borders
        int estimatedHeight = consoleHeightChars * fontHeight +
                              60; // 60px for title bar + borders

        std::cout << "[DEBUG] Estimated console size: " << estimatedWidth << "x"
                  << estimatedHeight << " (font: " << fontWidth << "x"
                  << fontHeight << ")" << std::endl;
        ...
        EnumWindows(
            [](HWND hwnd, LPARAM lParam) -> BOOL {
              ...
              if (GetWindowRect(hwnd, &rect)) {
                ...
                // Look for window with dimensions close to our estimate (within
                // 30%)
                if (w > 0 && h > 0) {
                  int widthDiff = abs(w - data->targetWidth);
                  int heightDiff = abs(h - data->targetHeight);

                  if (widthDiff < data->targetWidth * 0.3 &&
                      heightDiff < data->targetHeight * 0.3) {
                    char title[512];
                    GetWindowTextA(hwnd, title, sizeof(title));

                    std::cout << "[DEBUG] Found window with similar size: \""
                              << title << "\" (" << w << "x" << h
                              << ", HWND: " << hwnd << ")" << std::endl;
        ...
        if (findData.foundHwnd) {
          std::cout << "[GUI] Found console window by size matching, "
                       "attempting capture..."
                    << std::endl;
          hasConsole = CaptureWindowToBuffer(findData.foundHwnd, consoleBuffer,
                                             consoleWidth, consoleHeight);
        }
      }

      std::cout << "[GUI] Console window capture: "
                << (hasConsole ? "SUCCESS" : "FAILED") << std::endl;
      if (hasConsole) {
        std::cout << "[GUI] Console dimensions: " << consoleWidth << "x"
                  << consoleHeight << std::endl;
      }
    } else {
      std::cout << "[GUI] Console window is not visible, skipping capture"
                << std::endl;
    }
  } else {
    std::cout
        << "[GUI] No console window found (GetConsoleWindow returned NULL)"
        << std::endl;
  }
  ...
  if (!hasConsole) {
    stbi_write_png(filename, guiWidth, guiHeight, 4, guiBuffer.data(),
                   guiWidth * 4);
    std::cout << "[GUI] Screenshot saved (GUI only) to " << filename
              << std::endl;
    return;
  }

  if (!hasGui) {
    stbi_write_png(filename, consoleWidth, consoleHeight, 4,
                   consoleBuffer.data(), consoleWidth * 4);
    std::cout << "[GUI] Screenshot saved (Console only) to " << filename
              << std::endl;
    return;
  }
  ...
  std::vector<unsigned char> compositeBuffer(
      compositeWidth * compositeHeight * 4, 0);
  ...
  stbi_write_png(filename, compositeWidth, compositeHeight, 4,
                 compositeBuffer.data(), compositeWidth * 4);

  std::cout << "[GUI] Composite screenshot saved to " << filename
            << " (GUI: " << guiWidth << "x" << guiHeight
            << ", Console: " << consoleWidth << "x" << consoleHeight << ")"
            << std::endl;
}

// Screenshot Helper (DirectX 11) - Legacy single-window capture
void SaveScreenshot(const char *filename) {
  if (!g_pSwapChain || !g_pd3dDevice || !g_pd3dDeviceContext)
    return;
  ...
  if (SUCCEEDED(hr)) {
    ...
    stbi_write_png(filename, width, height, channels, image_data.data(),
                   width * channels);
...
void GuiLayer::DrawTuningWindow(FFBEngine &engine) {
  ...
  float current_width =
      Config::Get().GetShowGraphs() ? CONFIG_PANEL_WIDTH : viewport->Size.x;
  ...
  if (ImGui::Button("Close")) {
    if (Config::Get().IsPresetDirty(Config::Get().GetSelectedPresetIndex())) {
      std::cout << "[GUI] Unsaved changes detected on exit" << std::endl;
      show_unsaved_preset_warning = true;
    } else {
      std::cout << "[GUI] Exiting application via Close button" << std::endl;
      Config::Get().Save(engine);
      g_running = false;
    }
  }
  ImGui::Separator();
  ...
  if (devices.empty()) {
    ...
    if (selected_device_idx == -1 &&
        !Config::Get().GetLastDeviceGUID().empty()) {
      GUID target =
          DirectInputFFB::StringToGuid(Config::Get().GetLastDeviceGUID());
  ...
  if (ImGui::BeginCombo("FFB Device",
                        selected_device_idx >= 0
                            ? devices[selected_device_idx].name.c_str()
                            : "Select Device...")) {
    for (int i = 0; i < devices.size(); i++) {
      ...
      if (ImGui::Selectable(devices[i].name.c_str(), is_selected)) {
        selected_device_idx = i;
        DirectInputFFB::Get().SelectDevice(devices[i].guid);
        Config::Get().SetLastDeviceGUID(
            DirectInputFFB::GuidToString(devices[i].guid));
        // Config::Get().Save(engine);
      }
      if (is_selected)
  ...
  if (DirectInputFFB::Get().IsActive()) {
    if (DirectInputFFB::Get().IsExclusive()) {
      ImGui::TextColored(ImVec4(0.4f, 1.0f, 0.4f, 1.0f),
                         "Mode: EXCLUSIVE (Game FFB Blocked)");
      if (ImGui::IsItemHovered())
        ImGui::SetTooltip(
            "lmuFFB has exclusive control.\nThe game can read steering but "
            "cannot send FFB.\nThis prevents 'Double FFB' issues.");
    } else {
      ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.4f, 1.0f),
                         "Mode: SHARED (Potential Conflict)");
      if (ImGui::IsItemHovered())
        ImGui::SetTooltip("lmuFFB is sharing the device.\nEnsure In-Game FFB "
                          "is disabled\nto avoid LMU reacquiring the device.");
    }
  }

  bool always_on_top = Config::Get().GetAlwaysOnTop();
  bool auto_save = Config::Get().GetAutoSave();
  if (ImGui::Checkbox("Always on Top", &always_on_top)) {
    SetWindowAlwaysOnTop(g_hwnd, always_on_top);
    Config::Get().SetAlwaysOnTop(always_on_top);
    Config::Get().Save(engine);
  }
  ImGui::SameLine();
  if (ImGui::Checkbox("Auto-Save on Change", &auto_save)) {
    Config::Get().SetAutoSave(auto_save);
    Config::Get().Save(engine);
  }
  ImGui::SameLine();

  // --- B. THE CHECKBOX LOGIC (v0.5.5 Reactive Resize) ---
  bool toggled = Config::Get().GetShowGraphs();
  if (ImGui::Checkbox("Graphs", &toggled)) {
    // 1. Save the geometry of the OLD state before switching
    SaveCurrentWindowGeometry(Config::Get().GetShowGraphs());
    // 2. Update state
    Config::Get().SetShowGraphs(toggled);

    // 3. Apply geometry of the NEW state
    int target_w = Config::Get().GetShowGraphs()
                       ? Config::Get().GetWinWidthLarge()
                       : Config::Get().GetWinWidthSmall();
    int target_h = Config::Get().GetShowGraphs()
                       ? Config::Get().GetWinHeightLarge()
                       : Config::Get().GetWinHeightSmall();

    // Resize the OS window immediately
    ResizeWindow(g_hwnd, Config::Get().GetWinPosX(), Config::Get().GetWinPosY(),
                 target_w, target_h);

    // Force immediate save of state
    if (Config::Get().GetAutoSave()) {
      Config::Get().Save(engine);
    }
  }
  ...
  // base_nm: The physical force this effect produces at Gain 1.0 (Physics
  // Constant)
  ...
  auto FloatSetting = [&](const char *label, float *v, float min, float max,
                          float *config_param, const char *fmt = "%.2f",
                          const char *tooltip = nullptr,
                          std::function<void()> decorator = nullptr) {
    GuiWidgets::Result res =
        GuiWidgets::Float(label, v, min, max, fmt, tooltip, decorator);
    if (res.changed) {
      Config::Get().SetPresetDirty(selected_preset, true);
      *config_param = *v;
    }
    if (res.deactivated && Config::Get().GetAutoSave()) {
      Config::Get().Save(engine);
    }
  };

  auto BoolSetting = [&](const char *label, bool *v, bool *config_param,
                         const char *tooltip = nullptr) {
  ...
  auto IntSetting = [&](const char *label, int *v, const char *const items[],
                        int items_count, int *config_param,
                        const char *tooltip = nullptr) {
    GuiWidgets::Result res =
        GuiWidgets::Combo(label, v, items, items_count, tooltip);
    if (res.changed) {
      Config::Get().SetPresetDirty(selected_preset, true);
      *config_param = *v;
    }
  ...
  if (ImGui::TreeNodeEx("Presets and Configuration",
                        ImGuiTreeNodeFlags_DefaultOpen |
                            ImGuiTreeNodeFlags_Framed)) {

    auto presets = Config::Get().GetPresets();

    std::string preset_name = Config::Get().GetSelectedPresetName();
    const char *preview_value = preset_name.c_str();

    ImGui::SetNextItemWidth(ImGui::GetContentRegionAvail().x * 0.6f);
    if (ImGui::BeginCombo(
            Config::Get().IsPresetDirty(selected_preset) &&
                    !Config::Get().IsPresetBuiltin(selected_preset)
                ? "Preset*"
                : "Preset",
            preview_value)) {
      for (int i = 0; i < presets.size(); i++) {
        ...
        if (ImGui::Selectable(presets[i].name.c_str(), is_selected)) {
          selected_preset = i;
          Config::Get().ApplyPreset(i, engine);
        }
        if (is_selected)
          ImGui::SetItemDefaultFocus();
      }
    ...
    if (ImGui::Button("Save New")) {
      if (strlen(new_preset_name) > 0) {
        selected_preset =
            Config::Get().AddUserPreset(std::string(new_preset_name), engine);
        Config::Get().SetSelectedPresetName(new_preset_name);
        Config::Get().SetPresetDirty(selected_preset, false);
        new_preset_name[0] = '\0';
      }
    }

    if (!Config::Get().IsPresetBuiltin(preset_name)) {
      if (Config::Get().IsPresetDirty(selected_preset) &&
          !Config::Get().GetAutoSave() &&
          ImGui::Button("Save Current Config")) {
        Config::Get().Save(engine);
        Config::Get().SetPresetDirty(selected_preset, false);
      }
      if (ImGui::Button("Delete Preset")) {
        Config::Get().DeletePreset(selected_preset);
        selected_preset = 0;
        Config::Get().ApplyPreset(0, engine);
        Config::Get().SetPresetDirty(selected_preset, false);
      }
    } else {
      ImGui::SameLine();
  ...
  auto preset = Config::Get().GetPreset(selected_preset);
  if (!preset) {
    ImGui::TextColored(ImVec4(1, 0, 0, 1), "Error: Invalid preset index!");
    ImGui::End();
    return;
  }

  // --- GROUP: GENERAL ---
  if (ImGui::TreeNodeEx("General FFB", ImGuiTreeNodeFlags_DefaultOpen |
                                           ImGuiTreeNodeFlags_Framed)) {
    ImGui::NextColumn();
    ImGui::NextColumn();

    BoolSetting(
        "Invert FFB Signal", &engine.m_invert_force, &preset->invert_force,
        "Check this if the wheel pulls away from center instead of aligning.");
    FloatSetting("Master Gain", &engine.m_gain, 0.0f, 2.0f, &preset->gain,
                 FormatPct(engine.m_gain),
                 "Global scale factor for all forces.\n100% = No "
                 "attenuation.\nReduce if experiencing heavy clipping.");
    FloatSetting("Max Torque Ref", &engine.m_max_torque_ref, 1.0f, 200.0f,
                 &preset->max_torque_ref, "%.1f Nm",
                 "The expected PEAK torque of the CAR in the game.\nGT3/LMP2 "
                 "cars produce 30-60 Nm of torque.\nSet this to ~40-60 Nm to "
                 "prevent clipping.\nHigher values = Less Clipping, Less "
                 "Noise, Lighter Steering.\nLower values = More Clipping, More "
                 "Noise, Heavier Steering.");
    FloatSetting(
        "Min Force", &engine.m_min_force, 0.0f, 0.20f, &preset->min_force,
        "%.3f",
        "Boosts small forces to overcome the mechanical friction/deadzone of "
        "gear/belt driven wheels.\nPrevents the 'dead center' "
  ...
  if (ImGui::TreeNodeEx("Front Axle (Understeer)",
    ...
    FloatSetting(
        "Steering Shaft Gain", &engine.m_steering_shaft_gain, 0.0f, 2.0f,
        &preset->steering_shaft_gain, FormatPct(engine.m_steering_shaft_gain),
        "Scales the raw steering torque from the physics engine.\n100% = 1:1 "
        "with game physics.\nLowering this allows other effects (SoP, Vibes) "
        "to stand out more without clipping.");

    FloatSetting(
        "Steering Shaft Smoothing", &engine.m_steering_shaft_smoothing, 0.000f,
        0.100f, &preset->steering_shaft_smoothing, "%.3f s",
        "Low Pass Filter applied ONLY to the raw game force (Steering Shaft "
        "Gain).\nSmoothes out grainy or noisy signals from the game engine.",
        [&]() {
          int ms = (int)(engine.m_steering_shaft_smoothing * 1000.0f + 0.5f);
          ImVec4 color = (ms < LATENCY_WARNING_THRESHOLD_MS)
                             ? ImVec4(0, 1, 0, 1)
                             : ImVec4(1, 0, 0, 1);
          ImGui::TextColored(color, "Latency: %d ms - %s", ms,
                             (ms < LATENCY_WARNING_THRESHOLD_MS) ? "OK"
                                                                 : "High");
        });

    // Display with percentage format for better clarity
    FloatSetting("Understeer Effect", &engine.m_understeer_effect, 0.0f, 2.0f,
                 &preset->understeer, FormatPct(engine.m_understeer_effect),
                 ...
                 "  → First INCREASE 'Optimal Slip Angle' setting in the "
                 "Physics section.\n"
                 "  → Then reduce this slider if still too sensitive.\n\n"
                 "Technical: Force = Base * (1.0 - GripLoss * Effect)");

    const char *base_modes[] = {"Native (Steering Shaft Torque)",
                                "Synthetic (Constant)", "Muted (Off)"};
    IntSetting(
        "Base Force Mode", &engine.m_base_force_mode, base_modes,
        sizeof(base_modes) / sizeof(base_modes[0]), &preset->base_force_mode,
        "Debug tool to isolate effects.\nNative: Normal Operation.\nSynthetic: "
        "Constant force to test direction.\nMuted: Disables base physics (good "
        "for tuning vibrations).");
    ...
    if (ImGui::TreeNodeEx("Signal Filtering", ImGuiTreeNodeFlags_DefaultOpen)) {
      ...
      BoolSetting(
          "  Flatspot Suppression", &engine.m_flatspot_suppression,
          &preset->flatspot_suppression,
          "Dynamic Notch Filter that targets wheel rotation "
      ...
      if (engine.m_flatspot_suppression) {
        FloatSetting("    Filter Width (Q)", &engine.m_notch_q, 0.5f, 10.0f,
                     ...
                     "bandwidth (surgical removal).\nLower = Wider bandwidth "
                     "(affects surrounding frequencies).");
        FloatSetting("    Suppression Strength", &engine.m_flatspot_strength,
                     0.0f, 1.0f, &preset->flatspot_strength, "%.2f",
                     "How strongly to mute the flatspot vibration.\n1.0 = 100% "
                     "removal.");
        ImGui::Text("    Est. / Theory Freq");
        ImGui::NextColumn();
        ImGui::TextDisabled("%.1f Hz / %.1f Hz", engine.m_debug_freq,
                            engine.m_theoretical_freq);
        ImGui::NextColumn();
      }

      BoolSetting("  Static Noise Filter", &engine.m_static_notch_enabled,
                  &preset->static_notch_enabled,
                  "Fixed frequency notch filter to remove hardware resonance "
                  "or specific noise.");
      if (engine.m_static_notch_enabled) {
        FloatSetting("    Target Frequency", &engine.m_static_notch_freq, 10.0f,
                     100.0f, &preset->static_notch_freq, "%.1f Hz",
                     "Center frequency to suppress.");
        FloatSetting("    Filter Width", &engine.m_static_notch_width, 0.1f,
                     10.0f, &preset->static_notch_width, "%.1f Hz",
                     "Bandwidth of the notch filter.\nLarger = Blocks more "
                     "frequencies around the target.");
      }
  ...
  // --- GROUP: REAR AXLE ---
  if (ImGui::TreeNodeEx("Rear Axle (Oversteer)",
                        ImGuiTreeNodeFlags_DefaultOpen |
                            ImGuiTreeNodeFlags_Framed)) {
    ImGui::NextColumn();
    ImGui::NextColumn();
    ...
    FloatSetting(
        "Lateral G Boost (Slide)", &engine.m_oversteer_boost, 0.0f, 4.0f,
        &preset->oversteer_boost, FormatPct(engine.m_oversteer_boost),
        "Increases the Lateral G (SoP) force when the rear tires lose "
        "grip.\nMakes the car feel heavier during a slide, helping you judge "
        "the momentum.\nShould build up slightly more gradually than Rear "
        "Align Torque,\nreflecting the inertia of the car's mass swinging "
        "out.\nIt's a sustained force that tells you about the magnitude of "
        "the slide\nTuning Goal: The driver should feel the direction of the "
        "counter-steer (Rear Align)\nand the effort required to hold it "
        "(Lateral G Boost).");
    FloatSetting(
        "Lateral G", &engine.m_sop_effect, 0.0f, 2.0f, &preset->sop,
        FormatDecoupled(engine.m_sop_effect, FFBEngine::BASE_NM_SOP_LATERAL),
        "Represents Chassis Roll, simulates the weight of the car leaning in "
        "the corner.");
    FloatSetting(
        "SoP Self-Aligning Torque", &engine.m_rear_align_effect, 0.0f, 2.0f,
        &preset->rear_align_effect,
        FormatDecoupled(engine.m_rear_align_effect,
                        FFBEngine::BASE_NM_REAR_ALIGN),
        "Counter-steering force generated by rear tire slip.\nShould build up "
        "very quickly after the Yaw Kick, as the slip angle develops.\nThis is "
        "the active \"pull.\"\nTuning Goal: The driver should feel the "
        "direction of the counter-steer (Rear Align)\nand the effort required "
        "to hold it (Lateral G Boost).");
    FloatSetting(
        "Yaw Kick", &engine.m_sop_yaw_gain, 0.0f, 1.0f, &preset->sop_yaw_gain,
        FormatDecoupled(engine.m_sop_yaw_gain, FFBEngine::BASE_NM_YAW_KICK),
        "This is the earliest cue for rear stepping out. It's a sharp, "
        "momentary impulse that signals the onset of rotation.\nBased on Yaw "
        "Acceleration.");
    FloatSetting(
        "  Activation Threshold", &engine.m_yaw_kick_threshold, 0.0f, 10.0f,
        &preset->yaw_kick_threshold, "%.2f rad/s²",
        "Minimum yaw acceleration required to trigger the kick.\nIncrease to "
        "filter out road noise and small vibrations.");

    FloatSetting(
        "  Kick Response", &engine.m_yaw_accel_smoothing, 0.000f, 0.050f,
        &preset->yaw_smoothing, "%.3f s",
    ...
    FloatSetting(
        "Gyro Damping", &engine.m_gyro_gain, 0.0f, 1.0f, &preset->gyro_gain,
        FormatDecoupled(engine.m_gyro_gain, FFBEngine::BASE_NM_GYRO_DAMPING),
        "Simulates the gyroscopic solidity of the spinning wheels.\nResists "
        "rapid steering movements.\nPrevents oscillation and 'Tank "
        "Slappers'.\nActs like a steering damper.");

    FloatSetting(
        "  Gyro Smooth", &engine.m_gyro_smoothing, 0.000f, 0.050f,
        &preset->gyro_smoothing, "%.3f s",
    ...
    FloatSetting(
        "SoP Smoothing", &engine.m_sop_smoothing_factor, 0.0f, 1.0f,
        &preset->sop_smoothing, "%.2f",
        "Filters the Lateral G signal.\nReduces jerkiness in the SoP effect.",
        [&]() {
          int ms =
              (int)((1.0f - engine.m_sop_smoothing_factor) * 100.0f + 0.5f);
          ImVec4 color = (ms < LATENCY_WARNING_THRESHOLD_MS)
                             ? ImVec4(0, 1, 0, 1)
                             : ImVec4(1, 0, 0, 1);
          ImGui::TextColored(color, "Latency: %d ms - %s", ms,
    ...
    FloatSetting("  SoP Scale", &engine.m_sop_scale, 0.0f, 20.0f,
                 &preset->sop_scale, "%.2f",
                 "Multiplies the raw G-force signal before limiting.\nAdjusts "
                 "the dynamic range of the SoP effect.");
  ...
  // --- GROUP: PHYSICS ---
  if (ImGui::TreeNodeEx("Grip & Slip Angle Estimation",
                        ImGuiTreeNodeFlags_DefaultOpen |
                            ImGuiTreeNodeFlags_Framed)) {
    ImGui::NextColumn();
    ImGui::NextColumn();
    ...
    FloatSetting("Slip Angle Smoothing", &engine.m_slip_angle_smoothing, 0.000f,
                 0.100f, &preset->slip_smoothing, "%.3f s",
                 "Applies a time-based filter (LPF) to the Calculated Slip "
                 "Angle used to estimate tire grip.\n"
                 "Smooths the high fluctuations from lateral and longitudinal "
                 "velocity,\nespecially over bumps or curbs.\n"
                 "Affects: Understeer effect, Rear Aligning Torque.",
                 [&]() {
                   int ms =
                       (int)(engine.m_slip_angle_smoothing * 1000.0f + 0.5f);
                   ImVec4 color = (ms < LATENCY_WARNING_THRESHOLD_MS)
                                      ? ImVec4(0, 1, 0, 1)
                                      : ImVec4(1, 0, 0, 1);
                   ImGui::TextColored(
                       color, "Latency: %d ms - %s", ms,
                       (ms < LATENCY_WARNING_THRESHOLD_MS) ? "OK" : "High");
                 });

    FloatSetting("Chassis Inertia (Load)", &engine.m_chassis_inertia_smoothing,
                 0.000f, 0.100f, &preset->chassis_smoothing, "%.3f s",
                 "Simulation time for weight transfer.\nSimulates how fast the "
                 "suspension settles.\nAffects calculated tire load "
                 "magnitude.\n25ms = Stiff Race Car.\n50ms = Soft Road Car.",
                 [&]() {
                   int ms = (int)(engine.m_chassis_inertia_smoothing * 1000.0f +
                                  0.5f);
    ...
    FloatSetting(
        "Optimal Slip Angle", &engine.m_optimal_slip_angle, 0.05f, 0.20f,
        &preset->optimal_slip_angle, "%.2f rad",
        ...
        "NOTE: If the wheel feels too light at the limit, INCREASE this "
        "value.\n"
        "Affects: Understeer Effect, Lateral G Boost (Slide), Slide Texture.");

    FloatSetting("Optimal Slip Ratio", &engine.m_optimal_slip_ratio, 0.05f,
                 0.20f, &preset->optimal_slip_ratio, "%.2f",
                 "The longitudinal slip ratio (0.0-1.0) where peak "
                 "braking/traction occurs.\n"
                 "Typical: 0.12 - 0.15 (12-15%).\n"
                 "Used to estimate grip loss under braking/acceleration.\n"
                 "Affects: How much braking/acceleration contributes to "
                 "calculated grip loss.");
    // ---------------------------------
  ...
  if (ImGui::TreeNodeEx("Braking & Lockup", ImGuiTreeNodeFlags_DefaultOpen |
                                                ImGuiTreeNodeFlags_Framed)) {
    ImGui::NextColumn();
    ImGui::NextColumn();

    BoolSetting("Lockup Vibration", &engine.m_lockup_enabled,
                &preset->lockup_enabled,
                "Simulates tire judder when wheels are locked under braking.");
    if (engine.m_lockup_enabled) {
      FloatSetting("  Lockup Strength", &engine.m_lockup_gain, 0.0f, 3.0f,
                   &preset->lockup_gain,
                   FormatDecoupled(engine.m_lockup_gain,
                                   FFBEngine::BASE_NM_LOCKUP_VIBRATION));
      FloatSetting("  Brake Load Cap", &engine.m_brake_load_cap, 1.0f, 10.0f,
                   &preset->brake_load_cap, "%.2fx",
                   "Scales vibration intensity based on tire load.\nPrevents "
                   "weak vibrations during high-speed heavy braking.");

      FloatSetting("  Vibration Pitch", &engine.m_lockup_freq_scale, 0.5f, 2.0f,
                   &preset->lockup_freq_scale, "%.2fx",
                   "Scales the frequency of lockup and wheel spin "
      ...
      // - Sensitivity: %.0f (0 decimals) - Integer values are sufficient for
      // threshold
      // - Bump Rejection: %.1f m/s (1 decimal) - Balances precision with
      // readability
      // - ABS Gain: %.2f (2 decimals) - Standard gain precision across all
      // effects
      ImGui::Separator();
      ...
      FloatSetting(
          "  Gamma", &engine.m_lockup_gamma, 0.1f, 3.0f, &preset->lockup_gamma,
          "%.1f",
          "Response Curve Non-Linearity.\n1.0 = Linear.\n>1.0 = Progressive "
          "(Starts weak, gets strong fast).\n<1.0 = Aggressive (Starts "
          "strong). 2.0=Quadratic, 3.0=Cubic (Late/Sharp)");
      FloatSetting("  Start Slip %", &engine.m_lockup_start_pct, 1.0f, 10.0f,
                   &preset->lockup_start_pct, "%.1f%%",
                   "Slip percentage where vibration begins.\n1.0% = Immediate "
                   "feedback.\n5.0% = Only on deep lock.");
      FloatSetting(
          "  Full Slip %", &engine.m_lockup_full_pct, 5.0f, 25.0f,
          &preset->lockup_full_pct, "%.1f%%",
          "Slip percentage where vibration reaches maximum intensity.");
      ...
      FloatSetting(
          "  Sensitivity", &engine.m_lockup_prediction_sens, 10.0f, 100.0f,
          &preset->lockup_prediction_sens, "%.0f",
          "Angular Deceleration Threshold.\nHow aggressively the system "
          "predicts a lockup before it physically occurs.\nLower = More "
          "sensitive (triggers earlier).\nHigher = Less sensitive.");
      FloatSetting("  Bump Rejection", &engine.m_lockup_bump_reject, 0.1f, 5.0f,
                   &preset->lockup_bump_reject, "%.1f m/s",
                   "Suspension velocity threshold.\nDisables prediction on "
                   "bumpy surfaces to prevent false positives.\nIncrease for "
                   "bumpy tracks (Sebring).");

      FloatSetting("  Rear Boost", &engine.m_lockup_rear_boost, 1.0f, 10.0f,
                   &preset->lockup_rear_boost, "%.2fx",
                   "Multiplies amplitude when rear wheels lock harder than "
                   "front wheels.\nHelps distinguish rear locking (dangerous) "
                   "from front locking (understeer).");
    }
    ...
    // ABS
    BoolSetting("ABS Pulse", &engine.m_abs_pulse_enabled,
                &preset->abs_pulse_enabled,
                "Simulates the pulsing of an ABS system.\nInjects "
                "high-frequency pulse when ABS modulates pressure.");
    if (engine.m_abs_pulse_enabled) {
      FloatSetting("  Pulse Gain", &engine.m_abs_gain, 0.0f, 10.0f,
                   &preset->abs_gain, "%.2f", "Intensity of the ABS pulse.");
      FloatSetting("  Pulse Frequency", &engine.m_abs_freq_hz, 10.0f, 50.0f,
                   &preset->abs_freq, "%.1f Hz",
                   "Rate of the ABS pulse oscillation.");
    }
  ...
  // --- GROUP: TEXTURES ---
  if (ImGui::TreeNodeEx("Tactile Textures", ImGuiTreeNodeFlags_DefaultOpen |
                                                ImGuiTreeNodeFlags_Framed)) {
    ImGui::NextColumn();
    ImGui::NextColumn();

    FloatSetting(
        "Texture Load Cap", &engine.m_texture_load_cap, 1.0f, 3.0f,
        &preset->texture_load_cap, "%.2fx",
        "Safety Limiter specific to Road and Slide textures.\nPrevents violent "
        "shaking when under high downforce or compression.\nONLY affects Road "
        "Details and Slide Rumble.");

    BoolSetting("Slide Rumble", &engine.m_slide_texture_enabled,
                &preset->slide_enabled,
                "Vibration proportional to tire sliding/scrubbing velocity.");
    if (engine.m_slide_texture_enabled) {
      FloatSetting("  Slide Gain", &engine.m_slide_texture_gain, 0.0f, 2.0f,
                   &preset->slide_gain,
                   FormatDecoupled(engine.m_slide_texture_gain,
                                   FFBEngine::BASE_NM_SLIDE_TEXTURE),
                   "Intensity of the scrubbing vibration.");
      FloatSetting("  Slide Pitch", &engine.m_slide_freq_scale, 0.5f, 5.0f,
                   &preset->slide_freq, "%.2fx",
                   "Frequency multiplier for the scrubbing sound/feel.\nHigher "
                   "= Screeching.\nLower = Grinding.");
    }

    BoolSetting("Road Details", &engine.m_road_texture_enabled,
                &preset->road_enabled,
                "Vibration derived from high-frequency suspension "
    ...
    if (engine.m_road_texture_enabled) {
      FloatSetting("  Road Gain", &engine.m_road_texture_gain, 0.0f, 2.0f,
                   &preset->road_gain,
                   FormatDecoupled(engine.m_road_texture_gain,
                                   FFBEngine::BASE_NM_ROAD_TEXTURE),
                   "Intensity of road details.");
    }

    BoolSetting(
        "Spin Vibration", &engine.m_spin_enabled, &preset->spin_enabled,
        "Vibration when wheels lose traction under acceleration (Wheel Spin).");
    if (engine.m_spin_enabled) {
      FloatSetting("  Spin Strength", &engine.m_spin_gain, 0.0f, 2.0f,
                   &preset->spin_gain,
                   FormatDecoupled(engine.m_spin_gain,
                                   FFBEngine::BASE_NM_SPIN_VIBRATION),
                   "Intensity of the wheel spin vibration.");
      FloatSetting("  Spin Pitch", &engine.m_spin_freq_scale, 0.5f, 2.0f,
                   &preset->spin_freq_scale, "%.2fx",
                   "Scales the frequency of the wheel spin vibration.");
    }

    FloatSetting(
        "Scrub Drag", &engine.m_scrub_drag_gain, 0.0f, 1.0f,
        &preset->scrub_drag_gain,
        FormatDecoupled(engine.m_scrub_drag_gain,
                        FFBEngine::BASE_NM_SCRUB_DRAG),
        "Constant resistance force when pushing tires laterally (Understeer "
        "drag).\nAdds weight to the wheel when scrubbing.");

    const char *bottoming_modes[] = {"Method A: Scraping",
                                     "Method B: Susp. Spike"};
    IntSetting("Bottoming Logic", &engine.m_bottoming_method, bottoming_modes,
               sizeof(bottoming_modes) / sizeof(bottoming_modes[0]),
               &preset->bottoming_method,
               "Algorithm for detecting suspension bottoming.\nScraping = Ride "
  ...
  if (ImGui::CollapsingHeader("Advanced Settings")) {
    ...
    if (ImGui::TreeNode("Stationary Vibration Gate")) {
      ImGui::TextWrapped(
          "Controls when vibrations fade out and Idle Smoothing activates.");

      float lower_kmh = engine.m_speed_gate_lower * 3.6f;
      if (ImGui::SliderFloat("Mute Below", &lower_kmh, 0.0f, 20.0f,
                             "%.1f km/h")) {
      ...
      if (ImGui::SliderFloat("Full Above", &upper_kmh, 1.0f, 50.0f,
                             "%.1f km/h")) {
  ...
  if (show_unsaved_preset_warning) {
    ImGui::OpenPopup("Unsaved Changes");
    if (ImGui::BeginPopup("Unsaved Changes",
                          ImGuiWindowFlags_AlwaysAutoResize)) {
      ImGui::Text("You have unsaved changes in the current preset.\n"
...
void SetWindowAlwaysOnTop(HWND hwnd, bool enabled) {
  ...
  // SWP_NOACTIVATE prevents stealing focus, SWP_FRAMECHANGED ensures style bits
  // are refreshed
...
const int PLOT_BUFFER_SIZE =
    (int)(PLOT_HISTORY_SEC * PHYSICS_RATE_HZ); // 4000 points
...
void GuiLayer::DrawDebugWindow(FFBEngine &engine) {
  ...
  ImGui::SetNextWindowPos(
      ImVec2(viewport->Pos.x + CONFIG_PANEL_WIDTH, viewport->Pos.y));

  // Size: Fill the rest of the width
  ImGui::SetNextWindowSize(
      ImVec2(viewport->Size.x - CONFIG_PANEL_WIDTH, viewport->Size.y));
  ...
  if (ImGui::CollapsingHeader("A. FFB Components (Output)",
                              ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    // Raw Front Grip with warning label and stats
    {
      ...
      snprintf(stats_label, sizeof(stats_label),
               "Raw Front Grip | Val: %.4f | Min: %.3f | Max: %.3f", current,
               min_val, max_val);

      ImGui::Text("%s", stats_label);

      ImGui::PlotLines("##RawGrip", plot_raw_grip.data.data(),
                       (int)plot_raw_grip.data.size(), plot_raw_grip.offset,
                       NULL, 0.0f, 1.2f, ImVec2(0, 40));
      if (ImGui::IsItemHovered())
        ImGui::SetTooltip("Raw Grip Fraction from Game API");
    }

    PlotWithStats("Raw Rear Grip", plot_raw_rear_grip, 0.0f, 1.2f,
                  ImVec2(0, 40), "Raw Rear Grip Fraction from Game API");
    ...
    PlotWithStats("Avg Front Lat PatchVel", plot_raw_front_lat_patch_vel, 0.0f,
                  20.0f, ImVec2(0, 40), "Lateral Velocity at Contact Patch");

    PlotWithStats("Avg Rear Lat PatchVel", plot_raw_rear_lat_patch_vel, 0.0f,
                  20.0f, ImVec2(0, 40),
                  "Lateral Velocity at Contact Patch (Rear)");

    PlotWithStats("Avg Front Long PatchVel", plot_raw_front_long_patch_vel,
                  -20.0f, 20.0f, ImVec2(0, 40),
                  "Longitudinal Velocity at Contact Patch (Front)");

    PlotWithStats("Avg Rear Long PatchVel", plot_raw_rear_long_patch_vel,
                  -20.0f, 20.0f, ImVec2(0, 40),
